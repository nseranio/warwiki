...
push(txt(ix + 6, iy + 156, 9, 500, C.muted, 'start', 'Low storage pressure supports upper-tract safety'));
push(txt(ix + 6, iy + 168, 9, 500, C.muted, 'start', 'Continence also requires a competent outlet.'));


// Keep the root tag short: image-size detects SVG within the first 1,000 bytes.
const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${W} ${H}" width="${W}" height="${H}" role="img" aria-label="Studer orthotopic neobladder" aria-describedby="diagram-description">
<defs>
...
fs.writeFileSync(out, require('./lib/metadata').withFigureMetadata(svg, 'studer-neobladder'));
