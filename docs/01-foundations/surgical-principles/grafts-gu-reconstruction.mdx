...
A graft is tissue transferred from a donor site to a recipient site **without its native blood supply**. Survival depends on re-establishment of perfusion from the recipient bed through imbibition and inosculation (see [Principles of GU Reconstruction](principles-gu-reconstruction)). Grafts are selected when the recipient bed is well-vascularized, when flap reconstruction is unnecessary or disproportionate to the defect, or when the surgeon requires specific tissue properties — particularly mucosal lining — that only certain donor sites provide.
