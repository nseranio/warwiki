...
- **Parallel work:** a session may delegate single pages to the `warwiki-auditor` subagent (Sonnet). Up to 8 may run in parallel on different pages (`audit.py record` is file-locked); watch for rate limits. Only the main session commits.
