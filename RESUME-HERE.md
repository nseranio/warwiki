...
5. Reconcile remaining OpenEvidence leads. Presence-only and deferred decisions are copied to the central survey JSON; they are not source verification. HoLEP Cochrane25 has now been checked against the primary2026 review and is already correctly represented. NeuroSAFE PROOF101 is a verified functional-preservation gap ready for a concise cancer-survivorship addition; its source/eligibility/effect limits are in the survey JSON. High-priority remaining leads include NEUROGED12, GenderCOS74/75, ACS82/AAST94, NeuroSAFE PROOF101 and SPIRIT102. Keep cancer updates within reconstructive/survivorship scope.
6. Resume reference metadata collection from the saved JSONL cache. The scan is paused at6,355 cached records and is partial. Missing Crossref registration and low title overlap require primary-source investigation; do not automatically replace citations or delete claims. Run `build-reference-title-signals.py` against fresh source and cache when needed.
