# WARWIKI — resume after the flight

Saved September 11, 2026 at the user's requested travel stopping point.
...
- Monthly maintenance prioritizes major guidelines, large trials, Cochrane reviews and practice-impacting findings. Epic remains paused.
...
1. Check latest Git commit, CI and Vercel deployment before any new publication. The newest clinical correction batch is saved in a local checkpoint commit, not yet pushed. Finish final lint/build checks and diff review, then publish and verify it. Typecheck, unit and maintenance checks passed. A production build also completed successfully during checkpoint preparation; repeat it after the final citation cleanup to verify the exact frozen source before publishing.
...
These pages were inspected but the following corrections have NOT yet been implemented. Finish from original sources, then log scoped changes.
...
Preserve unrelated `world-cup-next-week-pacific.ics`. Source handouts remain recoverable but omitted from builds. Do not recreate Clinic or verbose public evidence banners. Detailed review notes belong in internal metadata/reports. User paused active work for the flight; no new automatic wakeup was requested. Existing monthly schedules remain as authorized.
