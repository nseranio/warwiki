...
**Latest site direction:** remove Clinic quick access and the `/clinic` page; simplify public evidence information to the existing last-updated display. Audit every page for errors and currency with a page-by-page log. Prioritize major guidelines, large trials, Cochrane reviews and high-yield practice changes over isolated small studies. These changes are in progress; the earlier Clinic implementation below is a historical record, not a requirement to retain it. Preserve accurate source provenance internally without manufacturing clinician sign-off.


**Follow-up implementation:** see [implementation-followup.md](reports/2026-09-11/implementation-followup.md) for Clinic access, seven-pathway evidence tables, all 55 figure dispositions, imaging/renal corrections, Node 24 and production build safeguards. GitHub and Vercel sign-ins are complete. The user now permits browser/UI interaction. Monthly workflows are published on `main` through `76cadfd4`; the first literature collection and CI passed. Authorized deployment cleanup is in progress, with the live release, known-good rollback and active alias references protected. Historical snapshots below describe earlier releases.
