...
- **Maintenance:** prepared but unpublished monthly GitHub literature/link artifacts plus active local Codex automation `warwiki-monthly-evidence-update`. See [maintenance.md](reports/2026-09-11/maintenance.md) for laptop/app availability, missed-run catch-up and GitHub inactivity limitations.
- **Publication caveat:** GitHub workflow changes remain on local `codex/monthly-workflows` (`5ede68bb` and `682dda8f`), pending permission to add `workflow` scope to the existing GitHub CLI sign-in. Do not assume the GitHub collectors are running; the local monthly editor can run the published collector script directly. Keep the pending workflow commit off `main` until it can be pushed.
- **Deployment:** preserve npm `postbuild` and Vercel's explicit `npm run build`; this excludes handout copies and reduces output from 566.08 MB to 139.55 MB, with a 200 MB hard budget. Vercel retention was already 30 days in all four categories; historical deployments were not deleted.
