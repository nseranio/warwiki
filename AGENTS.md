...
**Current audit continuation, September 21:** Read [RESUME-HERE.md](RESUME-HERE.md) and [audit-control.json](reports/2026-09-20/audit-control.json). Useful content was recovered on 141 pages; the restored-pending-review bucket is empty. The ledger records **931 full current-page reads across 1,186 files**, leaving 255 without one; 263 partial claim reviews, one nonclinical disposition and **zero complete clinical clearances**. The latest [sacral neuromodulation/PNE review](reports/2026-09-20/sacral-neuromodulation-pne-targeted-review.md) corrects product-versus-guideline selection, temporary-lead duration/threshold, FI denominator interpretation and ROSETTA scope while retaining the original diagram and video. Full/current device manuals, original tables/figures and supplements remain open. Grouped lint, typecheck, 32 unit/58 maintenance tests and final build (131.67 MB; 97,559 rendered links/assets) passed. None of the local changes is published. Next: intradetrusor Botox and two pharmacology companions with current FDA label and pivotal sources. Follow content-preservation policy. **Defer commit/push/deployment until the user asks to resume.**
...
**Latest site direction:** Clinic quick access, the `/clinic` page and the verbose public evidence banner are removed. Preserve the ordinary page-last-updated display. Audit every page for errors and currency with a page-by-page log; that clinical review remains in progress. Prioritize major guidelines, large trials, Cochrane reviews and high-yield practice changes over isolated small studies. Earlier Clinic descriptions are historical. Preserve accurate source provenance internally without manufacturing clinician sign-off.


**Implementation and hosting:** GitHub and Vercel sign-ins are complete; browser/UI interaction is authorized. Earlier cleanup deleted 41 obsolete deployments while preserving live/rollback releases and aliases. Use [the resumed release record](reports/2026-09-11/resumed-release.md) for the latest inventory and checks. The earlier [implementation report](reports/2026-09-11/implementation-followup.md) records the figure, imaging, Node 24 and build improvements; its Clinic description is superseded by the user's removal request. Evidence maintenance is quarterly; technical link checks remain monthly.

---

## Previous handoff snapshot — September 11, 2026 (initial evidence and maintenance refresh)

Release `eccd245a` is live, with GitHub CI 34607685640 and Vercel deployment 2Quu6iDEhcmxuksPpZTXmfgfQSft successful. Read [implementation-followup.md](reports/2026-09-11/implementation-followup.md) for the completed Clinic, seven-pathway evidence, 55-figure, imaging, renal and build-safeguard work. The initial 29-article literature refresh is preserved in the dated reports. This is not clinician sign-off of every article.
...
- **Maintenance:** monthly GitHub literature/link workflows are published (`8881cd95`, `d592751e`, `76cadfd4`) alongside active local Codex automation `warwiki-monthly-evidence-update`. All workflows use Node 24. Literature collection runs on the first day of each month at 06:23 UTC; link checking follows at 07:13 UTC. See [maintenance.md](reports/2026-09-11/maintenance.md) for laptop/app availability, missed-run catch-up and GitHub inactivity limitations.
- **First workflow verification:** CI 34617018910 passed. Literature run 34617072232 passed and saved 1,618 metadata candidates across seven topics for June 3–September 11; these are screening candidates, not verified clinical updates. External-link run 34617068341 correctly reported two DOI 404s; repair and repeat verification are in progress.
- **Deployment:** preserve npm `postbuild` and Vercel's explicit `npm run build`; this excludes handout copies and reduces output from 566.08 MB to 139.55 MB, with a 200 MB hard budget. Vercel retention was already 30 days in all four categories. An inventory identified 41 obsolete deployment candidates for the authorized cleanup; final deletion and storage results remain to be recorded.
...
- Dev server: `npm start` (Docusaurus dev server; `.Codex/launch.json` configured to run this on port 3001 for preview tooling).
...
| `CHANGELOG.md` | Rolling session-history log (formerly in AGENTS.md). |
