...
### Bladder mucosa graft filled

- **[Bladder mucosa graft](docs/01-foundations/surgical-principles/grafts/bladder-mucosa.mdx)** built from stub. Reframed as a **niche-but-legitimate option** with the strongest contemporary indication being **complex / recurrent / radiation-induced VVF** (Brandt 1998 96.3% n=80, Ostad 1998 100% n=6, Sharifi-Aghdas 2002 93%, Vyas 2005 91%); historical urethroplasty role anchored on **Kinkead 1994 long-term n=95** with the 68% vs 12% meatal-problem signal driving the "onlay > tube" rule and **El-Sherbiny 2002 canine 37% vs 12% vs 62%** stricture-rate hierarchy. Vaginoplasty (Chiaramonte 2018), ureteral substitute (Greenberg 1983, Yifeng 2008 — preclinical). Acellular bladder matrix descendants (el-Kassaby 2008 RCT showing healthy-bed 89% vs unhealthy-bed 33% divergence; Pokrywczynska / Coutu / UROGRAFT). DeRosa 2015 SCC-22-yr post-childhood-hypospadias-BMG flag. 21 refs.

### GU-specific reconstruction principles section eliminated

The user's editorial decision: **the Treatment Atlas already covers domain-specific GU reconstruction principles better, so the foundations layer should not duplicate them**.

- **Deleted** `principles-gu-reconstruction.mdx` and `gu-anastomotic-technique.mdx`.
- **Moved** [radiation-tissue-effects.mdx](docs/01-foundations/surgical-principles/radiation-tissue-effects.mdx) under **Plastic Surgery Principles** (sidebar_position 22 → 7).
- **Removed** the entire "GU-Specific Reconstruction Principles" group from [surgical-principles/index.mdx](docs/01-foundations/surgical-principles/index.mdx); rewrote the intro paragraph to a two-group structure (Plastic Surgery Principles + General Surgical Technique). Pointer added directing readers to the [Treatment Atlas](docs/04-surgical-techniques/index.mdx) for domain-specific GU reconstruction principles.
- **Repointed 6 inbound cross-links** away from the deleted pages: 04i-tissue-transfer/index.mdx, pyeloplasty.mdx, ureteral-reimplantation.mdx, surgical-skills/index.mdx (2 places, since deleted), sutures.mdx, barbed-sutures.mdx (2 places).

### Named Operative Techniques page eliminated → 3 surviving stitches relocated to hidden-category pattern

The user's editorial decision: the page mixed eponymous GU stitches with general surgical-skills training resources, which muddied scope. The training resources are not GU-specific and don't belong in Foundations.

- **Deleted** `surgical-skills/index.mdx`, plus `ski-needle.mdx`, `van-velthoven-vua.mdx`, `sexi-stance-exchange.mdx`, and the four bowel-stitch pages (`connell-stitch`, `cushing-stitch`, `lembert-stitch`, `halsted-stitch`).
- **Removed** Surgical Skills from the Foundations top-level subsection list ([01-foundations/index.mdx](docs/01-foundations/index.mdx)).
- **Folder kept hidden** — `surgical-skills/_category_.json` rewritten with `className: sidebar-hidden-category` (and a placeholder position 99). The folder no longer appears in the Foundations sidebar; the 3 surviving stitch pages remain reachable through the operational-home cross-links below.
- **Three surviving stitch pages**, each linked from a single operational home:
  - **[Heaney stitch](docs/01-foundations/surgical-skills/heaney-stitch.mdx)** — linked from [vaginal-hysterectomy.mdx](docs/04-surgical-techniques/04g-prolapse-repair/apical/vaginal-hysterectomy.mdx)
  - **[Quilting stitch](docs/01-foundations/surgical-skills/quilting-stitch.mdx)** — linked from [grafts-gu-reconstruction.mdx](docs/01-foundations/surgical-principles/grafts-gu-reconstruction.mdx) (graft-bed apposition row of the take-determinants table) AND [urethral-reconstruction-principles.mdx](docs/04-surgical-techniques/04a-urethral-reconstruction/urethral-reconstruction-principles.mdx) (graft fixation paragraph)
  - **[Parker-Kerr stitch](docs/01-foundations/surgical-skills/parker-kerr-stitch.mdx)** — linked from [urinary-diversion-principles.mdx](docs/04-surgical-techniques/04c-urinary-diversion/urinary-diversion-principles.mdx) (detubularization section, two-layer hand-sewn stump-closure note)
- **Bowel anastomosis page cleanup** — the four bowel-stitch links (Connell, Cushing, Lembert, Halsted) in [bowel-anastomosis.mdx](docs/01-foundations/surgical-principles/bowel-anastomosis.mdx) had pointed to the deleted pages; converted to bold inline references (no longer hyperlinks). The Parker-Kerr cross-link was preserved.

### Surgical-skills training resources moved to Resources (kept together in both locations)

Per the user's preference to **keep the resources together as a single block** rather than split between curricula and video series, the same "Surgical Skills Training" section now appears in both Resources locations:

- **[Resources → Websites & Online Tools](docs/08-resources/websites-online-tools.mdx)** — new "Surgical Skills Training" section with 7 cards (UCSF Skills Lab, ATOSS, BBASS, Behind The Knife Boot Camp, Clean Scalpel, Basic Surgical Skills, WARWIKI playlist) using the page's existing `web-card` styling
- **[Resources → Surgical Video Atlases](docs/08-resources/surgical-video-atlases.mdx)** — same 7 cards mirrored, with a one-line "mirrored at" pointer to the websites page

The resources are intentionally duplicated rather than split because (1) they're a small enough set, and (2) users coming from either entry point benefit from seeing both curricula and video series together rather than chasing cross-links.

### Editorial pattern guidance reinforced

- **Hidden-category pattern for orphan-but-linked content** — when a folder's index page is removed but specific child pages remain useful as references linked from operational homes, set `className: sidebar-hidden-category` in `_category_.json`. The orphan-check script already exempts this pattern (current lint: 200 hidden-category pages, all linked).
- **Reference-from-operational-home rule** — eponymous techniques like Heaney, Quilting, and Parker-Kerr are best linked **from the procedure where they're used** (vag hyst, urethroplasty principles + grafts, urinary diversion principles) rather than maintained on a standalone catalog page. Avoids the "page that exists for the sake of existing" problem.
- **Lint discipline** — every restructure ends with a clean `npm run lint` (scope, citations, orphans, internal links all pass). When a deletion creates broken inbound links, repoint to the most appropriate alternative page or remove the cross-link entirely; do not leave stubs or "redirect" placeholders.

### File count delta

`npm run lint` file count fell **744 → 734** across these two cleanups (10 deletions: principles-gu-reconstruction, gu-anastomotic-technique, surgical-skills/index, ski-needle, van-velthoven-vua, sexi-stance-exchange, connell-stitch, cushing-stitch, lembert-stitch, halsted-stitch). All four lint checks (scope, citations, orphans, internal links) pass clean.

---

## Previous handoff snapshot — April 27, 2026 (flap and graft foundations buildout — round 2)

Long session focused entirely on the `01-foundations/surgical-principles/flaps/` and `grafts/` families. Six flap / graft pages were either filled from stub or substantially augmented with comparison tables, outcome-series tables, and decision matrices, all following WARWIKI's bleed-through-ref-filtering rule and the hub-vs-deep-dive editorial split.

### Flap pages

- **[Medial thigh flap family](docs/01-foundations/surgical-principles/flaps/medial-thigh.mdx)** — rewritten from a Fournier's-only fasciocutaneous page into the **family / framework page** for the medial-thigh donor region. Vascular-territory map (MCFA, profunda perforators, obturator, DEPA, SFA, internal pudendal); 9-variant classification table; deep-dives on the variants without dedicated pages — Wang fasciocutaneous (1987 9 × 20 cm landmark), Persichetti septo-fascio-cutaneous V-Y (2003), MCFAP perforator (Karsidag 2011, Huang 2015), PAP (Arquette 2022, Lavie 2024 vPAP, Sharp 2021 combined gracilis + PAP), pPMT / vPMT (Scaglioni 2018, Wishart 2021), adductor flap (Angrigiani 2001), short gracilis (Soper 1989); cross-links out to gracilis, Singapore, and PMTP propeller without duplicating their content; 16 refs.
- **[Singapore / pudendal-thigh flap](docs/01-foundations/surgical-principles/flaps/singapore-pudendal-thigh.mdx)** — augmented from outline to full deep-dive. Tham 2010 three-vascular-territory cadaveric description (posterior labial deep to Colles' / obturator superficial / external pudendal superficial → medial-design rule explaining apical necrosis); Giroux 2021 ischial-tuberosity 27 mm landmark; Jin 2009 four-perforator microdissection. Ten clinical applications (MRKH neovagina with Monstrey 100% / Selvaggi uterovaginal continuity / Selçuk liposuction-thinned / Uccella 2024; bimodal oncologic outcomes — Monstrey 100% non-irradiated vs Gleeson "disappointing" irradiated; Joseph 1997 pediatric n = 12; RVF; posterior urethral defects in scarred perineum; obstetric fistula low-resource Pope / Browning / Pope / Maljaars; vulvar oncologic with Han 2023 IPAP vs PAP/TUG 12.9% vs 37.5% wound complications; Toulouse Algorithm; post-radiation stenosis Clifton; Benito Y-V burn-contracture; Alammar Fournier's). Six technical refinements table; outcomes summary table; comparison table vs Martius / gracilis / VRAM / ALT; preserved the lotus-petal disambiguation admonition. 26 refs.
- **[PMTP propeller flap](docs/01-foundations/surgical-principles/flaps/pmtp-propeller.mdx)** — substantial expansion. Tokyo Consensus 2011 framework. **Vascular-anatomy table** consolidating five studies (Scaglioni 2015, Ahmadzadeh 2007, Largo 2020, DeLong 2014, Mohan 2017) with the 8–10 cm-from-pubic-crease hot-spot rule. **Design-variants table** disambiguating pPMT / vPMT / PMTP propeller / PMT propeller. Six application sections (perineal oncologic, scrotal Fournier's, vulvar, perianal, **Wishart 2020 quadruplet** combined-flap pelvic reconstruction, Weisberger 2023 irradiated genital). Outcomes summary table (5 series, 47 flaps, 100% survival, 0% major complications, 100% primary closure). Comparison table vs gracilis / VRAM / PAP / Singapore. **Reconstructive-algorithm decision table** keyed to defect characteristics. Cross-link to the broader propeller-flap framework. 22 refs.

### Graft pages — built up the entire grafts/ subdirectory from stubs

- **[Lingual mucosa graft (LMG)](docs/01-foundations/surgical-principles/grafts/lingual-mucosa.mdx)** — built from stub. AUA 2023 Strong Recommendation Grade A equivalence with BMG (RR 1.03; 95% CI 0.96–1.10). Anatomy with **Campos-Juanatey 2022 histological-comparison table** (identical thickness and vascular density, but thinner LMG epithelium / submucosa, thicker muscular layer, less adipose, narrower-and-longer dimensions). Kumar 2008 / Xu 2017 harvest technique with explicit boundaries, 9–12 cm unilateral / 16–20 cm bilateral dimensions, 18-min mean harvest. Six clinical applications: anterior strictures (Simonato 2006/2008, Xu 2017 long-segment, Lumen 2016 prospective LMG vs BMG, Aldaqadossi 2020 LS); long-segment / panurethral; LS (Aldaqadossi 88.2% at 5 yr); failed hypospadias (Li 2016, Hongyong 2017 LMG-vs-BMG equivalence); female stricture (Sharma 2010, Richard 2021); distal / meatal. **Donor-site morbidity time-course tables** (Day 3 / 2 wk / 6 mo / ≥ 12 mo) anchored on Lumen 2016 and Xu 2011 with the key pattern (LMG more early speech / eating / dysgeusia; BMG more oral tightness; convergence at 6 mo). LMG-vs-BMG comparison table; decision matrix. 20 refs.
- **[Labial mucosa graft (LaMG)](docs/01-foundations/surgical-principles/grafts/labial-mucosa.mdx)** — built from stub. **Disambiguation admonition**: labial = inner lower lip, NOT vulvar labia. Mental-nerve anatomy with **Toure 2023 three-type intralabial distribution** and **Won 2014** 50% branch overlap / 60% buccal-nerve anastomosis redundancy. Three-site dimensions table. Histology anchored on **Soave 2014** (oral mucosa retains its histological character on the urethra and is not overgrown by urothelium). Five clinical applications: **Jena 2025 n = 204 dorsal-onlay LaMG for female urethral stricture, 93.5% at 2 yr** (the strongest contemporary anchor); male anterior stricture (Dessanti 1995, Caldamone 1998); hypospadias with **Castagnetti 2009 OR 5.86 tubularized-graft signal driving the "use as onlay, not tube" rule**; VVF interposition (Taha 2025); neovagina (Wu, Lin, Grimsby, van Leeuwen). **Donor-site morbidity comparison table** vs cheek harvest anchored on **Kamp 2005** (5.9 vs 1.0 mo pain, 10.3 vs 0.85 mo numbness, 50% vs 92% satisfaction → authors abandoned lower lip for cheek harvest in adults), with the **Tekin 2025 pediatric exception** (objective Semmes-Weinstein + thermal testing showing no detectable deficit in children) and **Castagnetti 2008** 28% subclinical perioral sensory defect at 7.6 yr. Three-site comparison table; decision matrix. 23 refs.
- **[Penile / preputial skin graft](docs/01-foundations/surgical-principles/grafts/penile-preputial-skin.mdx)** — built from stub with **graft-vs-flap admonition** preserving the editorial split (this page = free graft; pedicled-flap content remains on the dedicated [penile-preputial.mdx](docs/01-foundations/surgical-principles/flaps/penile-preputial.mdx) flap page). Historical context with Barbagli 1998 92% dorsal-onlay anchor, Fuchs 2018 4.3% → 38.7% PU shift, Berg 2024 GURS 99% BMG / 95% prefer-BMG-over-flap. Kulkarni 2023 spiral preputial graft (5 cm cuff → 20 cm graft, 1/20 recurrence). **Five clinical applications** with the **graft-vs-graft RCT comparison table** (PeeBuSt 89% vs 91%, Alrefaey 2025 93.2% vs 97.9%, Dubey 2007 graft-vs-flap). Bracka staged hypospadias (Manasherova 2020 31% vs 20% complications). Cambareri 2016 25-yr onlay graft. Mathur 2014 predictors-of-failure table (DM RR 5.21, smoking RR 4.19). Contraindications anchored on **Kulkarni 2009 LS multicenter 215-pt** series and AUA hair-bearing-skin prohibition; oral dyskeratosis as a relative *indication* for preputial skin. 18 refs.
- **[Saphenous vein graft](docs/01-foundations/surgical-principles/grafts/saphenous-vein.mdx)** — built from stub. Reframed as Peyronie's PIVG-dominant with secondary applications spelled out and an explicit "no urogyn role" admonition. Peyronie's outcomes table (Montorsi 2000 80% complete straightening, Yurkanin 2001 +2.1 cm length, Austoni 2005 95% satisfaction with concomitant prosthesis, Danacıoğlu 2021 equivalence to BMG); Lue & El-Sakka 1998 H-incision technique. Penile revascularization (Wagenknecht historical, Sohn / Manning selection criteria). Kidney-transplant vascular salvage (Oertl 2007, Shimizu 2018 short-vein extension, gonadal / superficial femoral alternatives). Pelvic venous-injury repair (Dalton 1979). Aortorenal bypass (AHA 2022 statement, adult durability, pediatric aneurysmal-degeneration caveat → hypogastric preferred). IVC / iliac reconstruction (gray-zone admonition; bovine-pericardium 86% dominance; Brown 2015 spiral graft for contaminated fields; Gloviczki 1990 SVC). Experimental urethral / ureteral (Kim 2014 rabbit, Wolters 2010 pig). Donor-site morbidity table (Garland / Hassoun-Kheir / Paletta) and endoscopic-harvest 14.2% → 5% reduction. Summary applications table with status flag for each indication. 33 refs.
- **[Posterior auricular skin graft](docs/01-foundations/surgical-principles/grafts/posterior-auricular.mdx)** — built from stub. Anchored on the **Manoj 2009 single dedicated urologic series** (n = 35, 89% success, circumcision + oral submucous fibrosis from betel-nut / tobacco, 35% LS strictures, mean 8.9 cm). "Why postauricular?" anchored on **Tomita 2009** rich dermal vascularity comparable to scalp / sole, **Hexsel 2015** Vancouver Scar Scale 1.87/13, and **Erba 2009** donor-site morbidity profile (no glasses / hearing-aid interference). Comprehensive comparison table vs all 8 other graft sources. **Filipas 1999 / El-Sherbiny 2002** skin-vs-mucosa biology data (skin shows 20–40% shrinkage and more inflammation under urine exposure). Decision matrix; explicit no-urogyn-role admonition. 18 refs.
- **[Split-thickness skin graft (STSG)](docs/01-foundations/surgical-principles/grafts/stsg.mdx)** — built from stub into a comprehensive page combining clinical applications and a full harvest-technique deep-dive. Thickness-classification table (ultra-thin / thin / standard / thick) with key thickness-evidence (Guogienė 2018 RCT, Chacon 2020 ultra-thin safety, Schreiter 0.1 mm rule for urethral mesh graft). **Seven urologic applications** — AABP (Gül 2026 STSG-vs-FTSG equivalence; pannus-as-donor-site Fuller 2017; outpatient Figler 2020); Fournier's (Susini SR, Hayon orchidopexy + meshed STSG, Ye / Zhao NPWT data); penile cancer (NCCN, Parnham 177-pt 9.3% local recurrence, Falcone 88.2% 1-yr RFS, Garaffa LS 84%, Malone TODGA); salvage urethroplasty (Schreiter mesh graft, Dalpiaz 93% dorsal inlay, **Furr 2021 long-term 53% vs BMG 96.4% caveat**, AUA hair-bearing prohibition); genital lymphedema (Alwaal); foreign body (Kang 2026); HS (Hamad 2020 JAMA Surg). Three gynecologic applications (McIndoe Linder, vulvovaginal Höckel-Dornhöfer, cervicovaginal Zhang). STSG-vs-FTSG comparison table. **Donor-site selection table** with Kovacs 2017 RCT showing occipital-scalp superiority over femur. **Comprehensive harvest-technique deep-dive** — dermatome selection (Tehrani 2006 survey, Dargan 2025 scalpel-blade test), thickness, **Ash 2024 tumescent SR Level I evidence** with epinephrine 1:250,000 sweet spot per Ho 2018, **Beckett 2019 lubricant comparison table** (water-based surgical lubricant best; saline surprisingly effective; mineral oil incompatible with Meek), step-by-step with the 30–45° dermatome-angle pearl, sheet vs meshed vs Meek processing, **Sinha 2017 donor-site pain protocol**, **Cao 2022 NPWT-vs-bolster RCT** (90.2% → 97.2% take, irregular areas 81.7% → 97.6%), Liguori 2020 dermal-regeneration template strategy, common-pitfalls table. 50 refs.
- **[Full-thickness skin graft (FTSG)](docs/01-foundations/surgical-principles/grafts/ftsg.mdx)** — built from stub. **FTSG vs STSG biology section** anchored on Stekelenburg 2016 (~91.5% area at 1 yr; **trunk vs extremity 94% vs 76%, *p* = 0.036**), Rudolph 1979 myofibroblast biology, Yamaguchi 2000 keratinocyte activation. Six urologic applications: AABP (Monn 2019 100% take from escutcheon FTSG; **Jeng 2026 — all patients with graft loss had diabetes**; Gül 2026 FTSG-vs-STSG equivalence; Plamadeala 2026 multicentre n = 204); pediatric (Wu 2026 inner-preputial FTSG, Thompson 2006 inguinal); Fournier's penile defects (Biju 2023 "FTSG whenever possible for penile defects"); urethroplasty (Brannan 1976 84% tube graft; **Webster 1985 penile-skin 18/18 vs extrapenile 75% failure**; Meeks 2010 abdominal-wall 14.5% hair growth; Liu 2015 prior urethroplasty / DM / LS as predictors not graft type; **Chen 2013 combined BMG + FTSG bringing ≥ 6 cm bulbar failure 24% → 0%**); Peyronie's (largely supplanted; comparative data show equivalent BMG / TV / saphenous vein / collagen fleece outcomes); exstrophy-epispadias (Harris 2020 96% successful reconstruction). Five gynecologic applications: penile-inversion vaginoplasty (**Opsomer 2021 Ghent 384-pt 15-yr — 85.7% require FTSG; diabetes only independent revision risk factor**; Buncamper 2017 100-pt graft-vs-no-graft equivalence); revision vaginoplasty (Van der Sluis 2016 81% FTSG vs 91% intestinal); MRKH (Motta 2016 mesh FTSG); vaginal stenosis (Morley & DeLancey 1991 flank); vulvovaginal cancer. FTSG-vs-STSG comparison table; donor-site table; key takeaways highlighting **diabetes as the dominant patient-level risk factor across both AABP and vaginoplasty series**. 33 refs.

### Editorial pattern guidance reaffirmed in this session

- **Family / framework page vs deep-dive** — for donor regions or technique families with multiple distinct variants, use a **family page** (medial-thigh) that maps the territory + classification + deep-dives on variants without dedicated pages, then cross-links out to dedicated pages for variants that already have them (gracilis, Singapore, PMTP propeller). Avoids duplication and lets each page have a clear scope.
- **Graft vs flap split (penile / preputial)** — formalized the editorial split with an explicit admonition. Free graft (Bracka, dorsal-onlay graft, DOIG, Kulkarni spiral) lives on the [graft page](docs/01-foundations/surgical-principles/grafts/penile-preputial-skin.mdx); pedicled dartos flap (Quartey, TPIF, Orandi, McAninch) lives on the [flap page](docs/01-foundations/surgical-principles/flaps/penile-preputial.mdx).
- **Disambiguation admonitions** — used in the labial mucosa page to head off the labia-vulva confusion, in the singapore-pudendal-thigh page to keep the gluteal-fold lotus petal distinct, and in the saphenous vein page to flag the absence of any urogyn role.
- **Bleed-through reference filtering rule continues to dominate** — every assistant-pasted source dump arrived with refs from unrelated articles (Martius VVF refs in the Singapore page, Peyronie's refs in the STSG harvest deep-dive, female-urethroplasty vaginal-flap refs in the lingual page, RUF / RVF gracilis refs in the medial-thigh family page). All filtered to refs actually cited in body, then renumbered sequentially.
- **Hub citation strategy for comparison tables** — when comparing 5+ alternatives in a table, cite the original sources in the "References:" line below the table rather than peppering each cell with `<sup>` superscripts; keeps tables readable and citation-density verifiable.

### Continuation targets for the grafts subdirectory

Four graft pages remain stubbed:

- [intestinal-segments.mdx](docs/01-foundations/surgical-principles/grafts/intestinal-segments.mdx)
- [bladder-mucosa.mdx](docs/01-foundations/surgical-principles/grafts/bladder-mucosa.mdx)

Both are reachable via [grafts-gu-reconstruction.mdx](docs/01-foundations/surgical-principles/grafts-gu-reconstruction.mdx).

---

## Previous handoff snapshot — April 26, 2026 (fistula buildout + flap-foundations sweep)
