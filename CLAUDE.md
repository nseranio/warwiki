...
## Current Handoff - 2026-05-11 (later) — Lymphedema named-technique atlas + vulvar-recon consolidation + new foundations flap pages

Six named-technique atlas pages for genital lymphedema surgery, a vulvar-reconstruction atlas + DB overhaul under a hybrid "foundations is canonical, atlas only for region-specific named techniques" policy (10 redundant atlas pages collapsed back to foundations after initial expansion), four new foundations flap pages (**IPAP / TFL / DIEP / TRAM**), four FGM/C-specific atlas pages, and resolution of audit-identified misroutings (labial-flap / mons-pubis / PAP-DFAP / IGAP). **31 commits**. Build clean across **1,006 files**.

Highlights:

- **New lymphedema atlas pages (all retained)** — [3R / SCIP-LFT (Yamamoto)](docs/04-surgical-techniques/04e-genital-reconstruction/3r-scip-lft.mdx), [CHASCIP (Ciudad)](docs/04-surgical-techniques/04e-genital-reconstruction/chascip.mdx), [Complete Functional Lymphatic-System Pedicled Transfer (Abdelfattah)](docs/04-surgical-techniques/04e-genital-reconstruction/complete-functional-lymphatic-pedicled-transfer.mdx), [LYST (Yoshimatsu / Yamamoto)](docs/04-surgical-techniques/04e-genital-reconstruction/lyst.mdx), [SAPL / Liposuction (Brorson)](docs/04-surgical-techniques/04e-genital-reconstruction/sapl-liposuction.mdx), [BLOOM (Sim)](docs/04-surgical-techniques/04e-genital-reconstruction/bloom-bariatric-vlnt.mdx). Scrotal-DB rows re-pointed; ranges from index-series outcomes (Yamamoto 3R n = 7 / 22.7 mo with 0% recurrence and no compression, to Abdelfattah n = 26 / 44.9 mo with GLS 6.2 → 0.05) to cross-region concepts (LYST framework, BLOOM single-case bariatric + repurposed-gastroepiploic-VLNT).
- **New vulvar atlas pages (retained)** — [Vulvar Primary Closure](docs/04-surgical-techniques/04e-genital-reconstruction/vulvar-primary-closure.mdx), [Vulvar Skin Graft (STSG / FTSG)](docs/04-surgical-techniques/04e-genital-reconstruction/vulvar-skin-graft.mdx) (consolidated row), [Genito-Crural Island Perforator Flap (GCIPF)](docs/04-surgical-techniques/04e-genital-reconstruction/vulvar-gcipf.mdx), [Foldès Clitoral Reconstruction](docs/04-surgical-techniques/04e-genital-reconstruction/foldes-clitoral-reconstruction.mdx), [Mañero Vaginal Mucosal Graft](docs/04-surgical-techniques/04e-genital-reconstruction/manero-vaginal-graft.mdx), [aOAP Flap](docs/04-surgical-techniques/04e-genital-reconstruction/aoap-flap.mdx), [Fat Grafting for FGM/C Vulvar Scars](docs/04-surgical-techniques/04e-genital-reconstruction/fgm-fat-grafting.mdx).
- **Vulvar DB expansion + consolidation** — added IGAP / MCFAP / EPAP / PAP-DFAP / vPMT / TUG / TRAM / gluteal-fold / sigmoid rows; tightened all 30 `bestFor` strings to concise one-line indications; then consolidated 10 redundant vulvar atlas pages back to anchored foundations sections (lotus petal / Singapore / gluteal-fold / EPAP / MCFAP / medial-thigh-V-Y / gluteal-V-Y / pubolabial-V-Y / Limberg) per hybrid policy; deleted Sigmoid Colon Flap row.
- **New foundations flap pages** — [IPAP](docs/01-foundations/surgical-principles/flaps/ipap.mdx) (promoted from a vulvar atlas to broaden scope across vulvar / vaginal / perineal / anal / buttock / penoscrotal / pressure-sore territories), [TFL](docs/01-foundations/surgical-principles/flaps/tfl.mdx) (Hill / Nahai 1978 → Hubmer perforator anatomy → Höckel first-choice-for-inguinal-region vulvar defects → Nirmal 2011 prophylactic-after-LND data), [DIEP](docs/01-foundations/surgical-principles/flaps/diep.mdx) (Rozen branching classification, Meyerov perforasome, Negosanti Type-II first-line, Pividori multicenter, Qiu DIEP-vs-TRAM head-to-head), [TRAM](docs/01-foundations/surgical-principles/flaps/tram.mdx) (Hartrampf 1982 birth, Moon-Taylor anastomotic patterns, Hartrampf-Hallock-Holm zones, MS-0/1/2/DIEP-MS-3 classification, delay / supercharging / turbocharging, Knox DIEP-vs-pTRAM 12-y).
- **VRAM page** — added a brief TRAM-variant cross-link section.
- **Misrouting audit fixes** — Labial flap row → `labia-majora-fasciocutaneous` (was wrongly `martius`); Mons-pubis row → `epap#the-external-pudendal-flap-family` (was `v-y-advancement`); PAP / DFAP → `medial-thigh#profunda-artery-perforator-pap-flap` (was `posterior-thigh`); IGAP → `igap-gluteal-fold#part-2` (was the whole page); TFL → new foundations page (was `anterolateral-thigh`); DIEP × 2 → new foundations page (were `vram`); TRAM → new foundations page (was `vram`).
- **FGM/C atlas suite** — Foldès (95% of published cases, six-step technique, n = 2,938 with 51% orgasm at 1 y); Mañero (only published non-Foldès alternative — vaginal mucosal graft FSFI 16 → 29); aOAP (O'Dey perforator from genitocrural sulcus — FGM/C 36% of n = 119 + LSA 87% of n = 61); FGM/C fat grafting (Almadori 2025 n = 13 with VASS / FGSIS / FSFI / HADS all significant at 12 mo). Re-pointed all four FGM/C-Reconstruction DB rows from the principles page.

**Hybrid consolidation policy established**: foundations flap pages are the canonical home for vascular / anatomic / technique / outcomes content; per-region atlas pages stay only when they carry substantial region-specific algorithm or named-technique content that does not fit within a foundations subsection. The Toulouse-named GCIPF, the FGM/C-specific Foldès / Mañero / aOAP / fat-grafting techniques, and the vulvar Primary Closure / Skin Graft pages meet that criterion; the perforator-flap family (IPAP / EPAP / MCFAP / lotus petal / Singapore / gluteal-fold) and rectus-abdominis family (VRAM / TRAM / DIEP) do not — their region-specific content has been folded back into the foundations pages.

Full session detail in `CHANGELOG.md`. Verification: `npm run lint:links`, `npm run lint:citations`, `npm run typecheck`, `npm run build` all pass across 1,006 files.
