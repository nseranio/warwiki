...
- **Cantonese** (do next): written Cantonese = Traditional Han + Cantonese-specific chars → needs a **CJK embedded font** (Noto Sans **HK** or **TC**), the **translate-all-first → subset → render-all** flow like zh/ko (subset must cover the glyphs actually used, so can't render per-batch). NOT yet in `HANDOUT_LANGUAGES` — add a `{code, label, englishLabel}` entry first (pick a code, e.g. `yue`, label `粵語`). Write `_YUE_TRANSLATOR_GUIDE.md`, `render-yue.sh` (clone `render-ko.sh`, `--virtual-time-budget`), download+instance+`pyftsubset` the HK/TC face, add `@font-face` to `_handout.css` AND inject into the 25 inline-CSS masters.
