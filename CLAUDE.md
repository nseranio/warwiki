...
24 commits, all fast-forwarded to `main`. Lint/typecheck/build clean throughout; every figure verified by a headless-Chrome render before embedding.
...
**15 original SVG schematics + 6 verified public-domain plates** — schematics copyright-free, each authored as a re-runnable generator in **`scripts/diagrams/`** (diagrams-as-code), embedded with the house caption pattern, build-validated. Three reusable **engines** emerged:
