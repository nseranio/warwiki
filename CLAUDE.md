...
- Prepared GitHub collectors will save literature and link-check artifacts. Local Codex automation `warwiki-monthly-evidence-update` reviews and integrates meaningful verified changes on the second day of each month. It requires this computer/app running; catch up from the last completed update after missed runs.
- Publication status: release `eccd245a` is live; remote CI 34607685640 and Vercel deployment 2Quu6iDEhcmxuksPpZTXmfgfQSft succeeded. Public Clinic and updated OASI content were checked. The GitHub workflow changes are saved separately on local branch `codex/monthly-workflows` (`5ede68bb` and `682dda8f`), awaiting the user's GitHub workflow-scope authorization. They are not yet running in GitHub. The local editor can use the published collector script directly; see the runbook before activating cloud schedules.
...
- All 55 figures have an editorial/source-coverage and visual audit, matching generators, provenance/text equivalents and accessible enlargement. Clinician approval remains pending and is displayed explicitly; do not replace that with a fabricated review date.
