...
**September 25 follow-up (done):** cloud-voice guardrails (budget cap, Blob cache, origin check; still off until Vercel is configured), Video Library refresh (1,563 videos), and the AUGS documents (full text supplied; all 29 PDFs integrated across five audit batches, billing checked against the CMS CY2026 file) are committed. The voice was approved on September 25 and is now [STYLE.md](STYLE.md). Pass 1 (typography and words) has been applied to 968 pages (September 26); the rest go through the audit, which applies Pass 1 and Pass 2 together. Tooling: `scripts/voice/` (`pass1.py`, `sweep_select.py`, `sweep_record.py`, `voice-report.py`) and the `warwiki-voice-editor` agent. Pass 2 (sentence structure) happens only in the audit. Restoring the OpenAI article voice waits until the audit is finished. Remaining AUGS items are only unverifiable claims, listed under "AUGS full text needed" in [reports/audit-v2/open-items.md](reports/audit-v2/open-items.md).

**Audit workflow v2, September 23:** The audit now follows [AUDIT.md](AUDIT.md), run on **Sonnet 5 / High**, with Haiku for mechanical subtasks and Opus only for escalations. Start sessions with [AUDIT-PROMPT.md](AUDIT-PROMPT.md). Tooling: `scripts/audit/audit.py` (queue/status) and `scripts/audit/pubmed.py` (source checks). Status lives in `reports/audit-v2/status.json`; a page is done when its sources are checked and corrections are published. Publish each 3–5-page batch to `main`, committing only the touched files. The v1 files (audit-workflow.md, audit-control.json, page ledger, claim batches) are history and a source of prior checks. The first v2 session performs the one-time takeover steps in AUDIT.md.

**September 20 user correction — preserve useful content:** Manufacturer IFUs/catalogs, anatomy texts, technical papers, operative videos and established teaching are valid sources for the claims they actually support. Lack of an RCT or unfinished source re-review is NOT a reason to erase practical content. Correct specific errors; retain useful material and record unresolved checks internally. Never replace substantive pages with audit/removal notices. Follow [reports/2026-09-20/content-preservation-policy.md](reports/2026-09-20/content-preservation-policy.md). The September 19–20 blanket-removal approach is superseded; restoration is not new clinical clearance. Current recovery: [restoration release record](reports/2026-09-20/content-restoration-release.md).
...
**v1 audit history (September 11–21):** [RESUME-HERE.md](RESUME-HERE.md) and the [restoration release record](reports/2026-09-20/content-restoration-release.md) describe the v1 state and the 141-page content restoration. Use them as background only; follow AUDIT.md for current work.
...
- The published literature collector runs quarterly on March/June/September/December1 at06:23UTC; technical link checks run monthly on day1 at07:13UTC. Local Codex automation `warwiki-monthly-evidence-update` (legacy ID; quarterly name/schedule) reviews and integrates meaningful verified changes on day2 of those four months. The local editor requires this computer/app running; catch up from the last completed update after missed runs.
- Publication status: release `eccd245a` was verified live with successful CI 34607685640 and Vercel deployment 2Quu6iDEhcmxuksPpZTXmfgfQSft. Public Clinic and updated OASI content were checked. Workflow commits `8881cd95`, `d592751e` and `76cadfd4` are now pushed to `main`, using Node 24; CI 34617018910 passed. Literature run 34617072232 passed and saved 1,618 metadata candidates across seven topics for June 3–September 11, not 1,618 verified updates. External-link run 34617068341 correctly flagged two DOI 404s; repair and repeat verification are in progress.
...
- Vercel explicitly runs `npm run build` including `postbuild`; preserve these hooks. Output is approximately 140 MB versus 566 MB before. Main-only deployments and report-only skips reduce storage growth. All four live retention categories were already 30 days. A verified inventory identified 41 obsolete deployment candidates for the authorized cleanup; final deletion and storage results remain to be recorded.
...
- All 55 figures have an editorial/source-coverage and visual audit, matching generators, provenance/text equivalents and accessible enlargement. Clinician approval remains pending in the internal review record; do not fabricate a review date. Follow the latest user direction to simplify the public evidence display.
...
### Platform / video-library updates

- `npm run videos:sync` was run against the WARWIKI YouTube channel. The generated registry now reflects **140 playlists / 1,542 unique videos** in [videos.ts](src/data/videos.ts).
- [VideoCards.tsx](src/components/VideoCards.tsx) now supports non-YouTube direct video sources via optional `sourceUrl` / `sourceType`, while preserving the existing YouTube embed behavior.
- [custom.css](src/css/custom.css) adds a direct-video placeholder and source-link styling for non-YouTube video cards.
- Resource copy updated from **90+** to **140+ custom playlists** in [Resources](docs/08-resources/index.mdx) and [Videos & Surgical Atlases](docs/08-resources/surgical-video-atlases.mdx).
- `src/data/stats.json` was regenerated by `npm run build` and now reflects the current corpus count.

### Targeted article video updates

- [Michigan Four-Wall SSLF](docs/04-surgical-techniques/04g-prolapse-repair/anterior/michigan-four-wall.mdx) now embeds the peer-reviewed PMC supplementary operative video from Larson 2013 via the new direct-MP4-capable `VideoCards` path.
- [Glean Urodynamics System](docs/01-foundations/tools/biomaterials/adjunct-specialty/glean-urodynamics.mdx) now embeds the two WARWIKI / YouTube Glean videos from the `Evaluation: Ambulatory Urodynamics` playlist.
- [Ileal Ureter](docs/04-surgical-techniques/04d-upper-tract-reconstruction/interposition-graft/ileal-ureter.mdx) had its old single video removed and now links to the Grand Rounds in Urology Ziho Lee robotic ileal ureter talk, links to the ScienceDirect open-access surgical film "Ileal Ureter Creation for Long-Segment Ureteral Injury," and embeds `j4nAm9rOxKg` (`Core Videos (2024): Single Position Robotic-Assisted Laparoscopic Right Ileal Ureter Interposition`).

### Notes

- The Grand Rounds robotic ileal ureter page is linked as an external video-resource card rather than embedded.
- The PMC page itself may be browser-challenge protected, but the direct supplement MP4 URL was incorporated exactly as provided.

---

## Previous Handoff - 2026-06-16 — Social assets: patient-handouts language announcement

Off-repo social package finalized at `~/Desktop/WARWIKI-social/handouts-languages/` for announcing **80 patient guides in 10 languages**. Final directory index saved as `HANDOFF.md`.

### Instagram single post

- Final image: `single-better.png` (`2160x2700`), with editable/source siblings `single-better.svg`, `single-better.html`, and `single-better.js`.
- Final design: portrait single-post layout, concise headline ("80 patient guides in 10 languages"), short support line ("Free, printable, plain-language PDFs."), large language/flag tiles, CTA pill (`Read & print free -> warwiki.org`), no bottom footer line, no "10 READY" badge.
- Wordmark was changed from the earlier serif treatment to a cleaner site-style sans WARWIKI wordmark, closer to the website/social-card branding.
- Caption saved in `caption.txt`; opening line now reads: "Take patient education to the next level with brand-new official WARWIKI patient handouts, now available in 10 languages."

### Twitter / X adaptation

- Final image: `twitter.png` (`3200x1800`), with editable/source siblings `twitter.svg`, `twitter.html`, and `twitter.js`.
- Layout is 16:9 landscape for feed readability: headline + CTA left, language panel right. Arabic label placement was fixed for RTL rendering in the SVG/Chrome export path.
- Tweet copy + alt text saved in `twitter-caption.txt`; tweet is under the X character limit.

### Durable notes

- Assets remain off-repo, following the existing `~/Desktop/WARWIKI-social/` convention for social graphics.
- Generator pattern: SVG-in-HTML from Node source, exported with headless Chrome at 2x scale for upload-safe PNGs.
- Language set shown: Spanish, Mandarin, Vietnamese, French, Korean, Tagalog, Arabic, Russian, Cantonese, Hindi.
- No repo code/build changes were made for the social assets themselves; this handoff/changelog update is the only repo documentation change.

---

## Previous Handoff - 2026-06-13 — Handouts: FULL Hindi (`hi`) localization (80/80, हिन्दी)
...
**Font (the headline finding — carry into Hindi):** written Cantonese = Traditional Han + Cantonese-specific chars (嘅/咗/喺/啲) that Noto Sans SC lacks, so `yue` embeds **Noto Sans HK, full face (NOT subset)**: google/fonts variable → `fontTools varLib.instancer` to static Reg/Bold (`fonts/NotoHK-Reg.ttf`/`NotoHK-Bold.ttf`, ~7 MB/face, off-repo). **Per-batch path PROVEN like Arabic** — Chrome `--print-to-pdf` auto-subsets the embedded HK face into each PDF (each `.yue.pdf` carries a `+NotoSansHK` FontFile2 subset, ~130–160 KB), so **NO `pyftsubset`, NO translate-all-first** (the old zh/ko CJK constraint does NOT apply when embedding the full face).
- **Font-ordering fix:** HK uses **Traditional** glyph forms → must NOT precede Noto Sans SC in the shared stack (would corrupt Simplified `zh`, which shares `_handout.css`). Added a **`[lang]`-scoped override** in `_handout.css`: `html[lang="yue"] body, html[lang="zh-HK"] body, html[lang="zh-Hant-HK"] body { font-family:'Noto Sans HK', … }`. `yue` masters set `<html lang="yue">`, LTR.
- **Inline-master wrinkle RESOLVED:** the 56 linked masters get HK for free via `_handout.css`. The **25 inline-`<style>` masters** don't link it, so for those the render step **injects the two HK `@font-face` rules after `<style>` + prepends `'Noto Sans HK'` to the body `font-family:'Inter'…` stack** (a one-shot Python pass before `render-yue.sh`). Verified the injected inline PDFs embed `NotoSansHK` and are gate-clean. (This also flags a latent risk for the **inline-master Arabic** sheets — `urethroplasty.ar.html` has no `@font-face` and doesn't link `_handout.css`, so its Arabic may be falling back; worth auditing the 25 inline `.ar.pdf` for embedded NotoArabic separately.)

**NEXT (user roadmap): Hindi (`hi`), then delete `it`/`ja` LAST.** Hindi = Devanagari → embedded **Noto Sans Devanagari** (instance Reg/Bold, `@font-face`, render with `--virtual-time-budget`; Chrome auto-subsets into the PDF so **no pre-`pyftsubset`** — verify one sheet like ar/yue), **LTR** so no RTL work. Add `hi` to `HANDOUT_LANGUAGES`. Reuse this exact loop + the inline-master font-injection pass. Roadmap in memory [[project_handout_language_roadmap]].
...
**NEXT (user roadmap, supersedes the old es…ja list): Cantonese → Hindi (`hi`), then delete the other coming-soon langs (Italian `it`, Japanese `ja`) LAST** (build first, delete last — don't waste tokens deleting early). Saved in memory [[project_handout_language_roadmap]].
- **Cantonese** (do next): written Cantonese = Traditional Han + Cantonese-specific chars → needs a **CJK embedded font** (Noto Sans **HK**, which covers the Cantonese-specific chars like 嘅/咗/喺/啲). NOT yet in `HANDOUT_LANGUAGES` — add a `{code, label, englishLabel}` entry first (e.g. `yue`, label `粵語`). Write `_YUE_TRANSLATOR_GUIDE.md`, `render-yue.sh` (clone `render-ar.sh`/ko, `--virtual-time-budget`), add `@font-face` to `_handout.css` AND inject into the 25 inline-CSS masters. **Font strategy — reconsider vs the old zh/ko translate-all-first rule:** the Arabic finding (Chrome `--print-to-pdf` auto-subsets the embedded face into each PDF) means you can likely **skip `pyftsubset` and render PER-BATCH** by embedding the full Noto Sans HK face — the old "must subset → translate-all-first" constraint was only because zh/ko pre-subset the @font-face file. Test one sheet first: if the per-batch PDF embeds the glyphs (check `strings <pdf> | grep NotoSansHK`), use the fast per-batch loop. Only fall back to pre-subset + translate-all-first if the full ~10MB HK `@font-face` load is too slow under `--virtual-time-budget`. Cantonese is compact (like zh) → expect little/no overflow trimming.
- **Hindi** (`hi`, last): Devanagari → embedded **Noto Sans Devanagari** (same as Arabic: instance Reg/Bold, `@font-face`, render with `--virtual-time-budget`; Chrome auto-subsets into the PDF so likely **no pre-`pyftsubset`** — verify one sheet first like ar), **LTR** so no RTL work. Add to `HANDOUT_LANGUAGES`.

**Reusable pipeline reference (Arabic = first RTL + first embedded-font-non-CJK; ru = latest verbose-Latin).** Durable findings, full detail in `CHANGELOG.md` under 2026-06-12:
...
- **Embedded-font render scripts** (`render-ar.sh`/CJK) clone `render-ko.sh` with `--headless=new --virtual-time-budget=20000` so the font loads before print → no blank PDF. Translator guides off-repo (formal register, concise, label map + glossary; `<bdi>` rule for Arabic). **Per-batch loop:** 10 Sonnet agents → `.<code>.html` → `./render-<code>.sh <slug>` (gate must read pageOvf=0/colsOvf=0) → `cp` PDFs to `static/handouts/` + JPGs to `static/img/handouts/` → `node scripts/add-lang.js <code> <slugs…>` → `npm run typecheck && npm run build` → `git checkout -- src/data/stats.json` → commit + push main. **Pitfall:** zsh doesn't word-split unquoted vars — drive render/copy loops with `while IFS= read -r s; … < file`, not `for s in $VAR`.
...
2. Voice: write for reconstructive surgeons and urogynecologists, not patients or general medicine readers. Keep operative relevance, anatomy, decision points, complications, and outcomes. Follow [STYLE.md](STYLE.md) for typography, word choice, sentence structure and evidence wording; new content, including quarterly Codex evidence updates, is written in that voice.
