...
- Publication status: release `a7768dd4` is live; remote CI passed. The GitHub workflow changes are saved separately on local branch `codex/monthly-workflows` (`5ede68bb`), awaiting the user's GitHub workflow-scope authorization. They are not yet running in GitHub. The local editor can use the published collector script directly; see the runbook before activating cloud schedules.
