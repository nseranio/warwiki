...
**September 25 follow-up (done):** cloud-voice guardrails (budget cap, Blob cache, origin check; still off until Vercel is configured), Video Library refresh (1,563 videos), and the AUGS documents (full text supplied; all 29 PDFs integrated across five audit batches, billing checked against the CMS CY2026 file) are committed. The voice was approved on September 25 and is now [STYLE.md](STYLE.md). Pass 1 (typography and words) may be applied with `scripts/voice/pass1.py` and the `warwiki-voice-editor` agent. Pass 2 (sentence structure) happens only in the audit. Restoring the OpenAI article voice waits until the audit is finished. Remaining AUGS items are only unverifiable claims, listed under "AUGS full text needed" in [reports/audit-v2/open-items.md](reports/audit-v2/open-items.md).
