...
**Audit workflow update, September 20:** Use **GPT-5.6 Sol / High** as clinical audit lead, with bounded shared-source batches and a focused source/diff review. Read [audit-workflow.md](reports/2026-09-20/audit-workflow.md) and [audit-control.json](reports/2026-09-20/audit-control.json) for the single writer and exact next action. Finish the existing audit before new OpenEvidence pulls; record later high-yield candidates in [the deferred refresh list](reports/2026-09-20/open-evidence-refresh-queue.md). Terra/Medium is limited to specified mechanical support. Existing read/update counts are not clinical completion.
