...
- **Phase 2, Video Library:** `npm run videos:sync` gives 143 playlists and 1,563 unique videos (21 added, 4 removed). Removed the deleted Peyronie's video `LcVV9zbXHHQ` from [plaque-incision-grafting](docs/04-surgical-techniques/04j-sexual-dysfunction/peyronies-disease/plaque-incision-grafting.mdx). Added cards to Martius and omental flap, EPA, ventral onlay BMG, PFUI, post-radiation posterior stenosis, ileocystoplasty, intracorporeal diversion, extravesical VVF and simple prostatectomy pages. Skipped: the French robotic vulvovaginoplasty video (title only, technique unverified), the transdiaphragmatic omental video (off-topic), and one bulbar obliterative-stricture video (no matching technique page).
- **Phase 3, AUGS documents (full text supplied September 25):** the AUGS documents in ~/Downloads were extracted to `reports/audit-v2/sources-local/augs-2026-09/` and each recommendation was checked against the text before it went on a page. Batches: GSM and vaginal estrogen (microfat grafting, GSM, vaginal estrogen, vaginal laser, OAB, IC/BPS); UTI, bacteriuria and hematuria (recurrent UTI, geriatric urology, urine studies, NLUTD stroke/dementia, urodynamics, antibiotic pages); prolapse and uterine preservation (prolapse principles, apical prolapse, hysteropexy, sacrohysteropexy, hysterectomy, sacrocolpopexy, slings, cancer survivorship); billing and finance; quick perioperative and instrument items. [Billing & Coding](docs/08-resources/hidden-curriculum/billing-coding.mdx): all 232 CPT wRVUs were checked against the CMS CY2026 relative value file (RVU26C, July 2026) and 227 were wrong and corrected; conversion factor $33.4009 ($33.5675 for qualifying APM participants); a dated coding and coverage notes section was added. Where the AUGS text contradicted the triage (Statement 20, ospemifene label, ReCOUP powering, elongation threshold, +99459 RVU) the document was followed.
- **Phase 4, voice tooling and pilot (nothing in docs/ changed):** `scripts/voice/` holds `pass1.py`, `check_voice_diff.py` and `voice-report.py` (`npm run lint:voice`, advisory). Round 3 pilot on 10 copies passed with no major drift; review file is `reports/voice-pilot-2026-09-25/REVIEW.md`. STYLE section 4 (evidence wording) was added to AUDIT.md step F.
...
- Pass 1 sweep (waves 1–11, September 25–26): 968 pages in the new voice (tier 4, tier 3, then the audited tier 1–2 core pages), all passing `check_voice_diff.py`; about 30% judged blind for meaning drift, with each major finding restored (deleted qualifiers, 'gold standard', a warning label, added connectives, essential/critical → 'required' softened to 'needed'). Site rates per 1,000 prose words: em dashes 9.3 → 1.0, bold 27.1 → 14.4, arrows 0.6 → 0.1, empty evaluation words 0.8 → 0.3. Progress and metrics are in `reports/voice/status.json`. Left for the audit (which applies Pass 1 and Pass 2 together): the 26 pages next in the audit queue and 5 with uncommitted audit edits, plus about 190 pages with fewer than 3 Pass 1 patterns. Pass 2 (sentence structure) has not been applied to any page yet; it happens inside the audit.
...
- **Tier 3 checked (perioperative care):** [Analgesia](docs/01-foundations/perioperative-care/intraoperative-care/analgesia.mdx), [Anesthesia](docs/01-foundations/perioperative-care/intraoperative-care/anesthesia.mdx), [Draping & Skin Antisepsis](docs/01-foundations/perioperative-care/intraoperative-care/draping-skin-antisepsis.mdx), [Nausea & Vomiting](docs/01-foundations/perioperative-care/intraoperative-care/nausea-vomiting.mdx), [Nerve Blocks](docs/01-foundations/perioperative-care/intraoperative-care/nerve-blocks.mdx), [Positioning Nerve Injury](docs/01-foundations/perioperative-care/intraoperative-care/positioning-nerve-injury.mdx), [Surgical Gloving](docs/01-foundations/perioperative-care/intraoperative-care/surgical-gloving.mdx), [Surgical Hand Antisepsis](docs/01-foundations/perioperative-care/intraoperative-care/surgical-hand-antisepsis.mdx), [Antithrombotic Therapy](docs/01-foundations/perioperative-care/perioperative-protocols/antithrombotic-therapy.mdx), [ERAS](docs/01-foundations/perioperative-care/perioperative-protocols/eras.mdx), [Constipation](docs/01-foundations/perioperative-care/postoperative-management/constipation.mdx).
- **Main corrections:** DOAC hold/resume timing rewritten to the PAUSE protocol; P2Y12 stop intervals corrected; DAS airway-attempt limits re-attributed to the 2015 guideline; lipid-emulsion bolus timing (2-3 min) and penile-block epinephrine wording aligned with ASRA and labels; wrong or unrelated references replaced (positioning page, nerve blocks); Feng hand-rub meta-analysis reported as colony-count evidence, not SSI; suzetrigine label and safety details; pediatric ondansetron dose range; Cochrane ERAS figures added. Positioning, nerve-block, anesthesia, hand-antisepsis and gloving pages regained content removed in v1.
- **Open:** ASRA 5th-edition (2025) interval table verified only through secondary summaries; DAS 2015 attempt limits, MHAUS dantrolene details and AHA/ACC 2024 stop days are not checkable from primary text (see `reports/audit-v2/open-items.md`).

---

## 2026-09-25 (later) — Audit v2 tier 2 partial pages completed

- **Completed to `checked`:** [Female Urethral Masses](docs/03-clinical-conditions/03b-voiding-outlet/female-urethral-masses.mdx) (malignancy and leiomyoma attributions, Yang 2020 counts, urethral carcinoma survival and prognostic citations), [NLUTD Spina Bifida](docs/03-clinical-conditions/03d-nlutd/nlutd-spina-bifida.mdx) (epidemiology and MOMS figures, augmentation/channel complication rates, tethered cord, bowel program, transition, pregnancy, latex), [Vaginal Cysts & Masses](docs/03-clinical-conditions/03g-genital-scrotal/vaginal-cysts-masses.mdx) (prevalence scoping, VAIN regression/recurrence figures, imiquimod evidence, FIGO stage line).
- **Tier 2 status:** 151 of 155 recorded `checked` or `not-clinical`. Remaining `partial`: two unbuilt placeholder pages (antispasmodics, dantrolene) and their two hubs, which wait on authoring.

---

## 2026-09-25 — Audit v2 tier 2, batch 11: genital/scrotal, pelvic pain and defecatory disorders; tier 2 complete

- **Checked:** genital and scrotal: [Buried Penis](docs/03-clinical-conditions/03g-genital-scrotal/buried-penis.mdx), [Clitoral Phimosis](docs/03-clinical-conditions/03g-genital-scrotal/clitoral-phimosis.mdx), [FGM/C](docs/03-clinical-conditions/03g-genital-scrotal/female-genital-mutilation-cutting.mdx), [Genital Lymphedema](docs/03-clinical-conditions/03g-genital-scrotal/genital-lymphedema.mdx), [Giant Penoscrotal Lymphedema](docs/03-clinical-conditions/03g-genital-scrotal/giant-penoscrotal-lymphedema.mdx), [Hidradenitis Suppurativa](docs/03-clinical-conditions/03g-genital-scrotal/hidradenitis-suppurativa.mdx), [Lichen Sclerosus](docs/03-clinical-conditions/03g-genital-scrotal/lichen-sclerosus.mdx), [Müllerian Anomalies](docs/03-clinical-conditions/03g-genital-scrotal/mullerian-anomalies.mdx), [Peyronie's Disease](docs/03-clinical-conditions/03g-genital-scrotal/peyronies-disease.mdx), [Small Penis Syndrome](docs/03-clinical-conditions/03g-genital-scrotal/small-penis-syndrome.mdx), [Testicular Reimplantation](docs/03-clinical-conditions/03g-genital-scrotal/testicular-reimplantation.mdx), [Vaginal Cysts & Masses](docs/03-clinical-conditions/03g-genital-scrotal/vaginal-cysts-masses.mdx) (`partial`); pelvic pain: [Chronic Pelvic Pain](docs/03-clinical-conditions/03h-pelvic-pain/chronic-pelvic-pain.mdx), [IC/BPS](docs/03-clinical-conditions/03h-pelvic-pain/ic-pbs.mdx), [Myofascial Pelvic Pain](docs/03-clinical-conditions/03h-pelvic-pain/myofascial-pelvic-pain.mdx), [Pelvic Venous Disorders](docs/03-clinical-conditions/03h-pelvic-pain/pelvic-venous-disorders.mdx); defecatory disorders: [index](docs/03-clinical-conditions/03i-defecatory-disorders/index.mdx), [Chronic Constipation](docs/03-clinical-conditions/03i-defecatory-disorders/chronic-constipation.mdx), [Defecatory Dysfunction](docs/03-clinical-conditions/03i-defecatory-disorders/defecatory-dysfunction.mdx), [Fecal Incontinence](docs/03-clinical-conditions/03i-defecatory-disorders/fecal-incontinence.mdx); [Clinical Conditions index](docs/03-clinical-conditions/index.mdx) (`not-clinical`, cards aligned to actual pages).
- **Main corrections:** buried penis success rates (78-91%, not 91-96%) and mis-cited rows; FGM/C deinfibulation timing and guideline split (RCOG vs WHO 2025); lichen sclerosus SCC absolute risks and EAU genital-skin recommendation; genital lymphedema unsupported cellulitis ORs replaced and PATCH I prophylaxis added; giant penoscrotal lymphedema series denominators; HS label doses; Müllerian prevalence and vaginoplasty complication scoping; Peyronie's prevalence, OR attribution and natural-history figures; small penis syndrome trial details; IC/BPS AUA grades and Welch RCT; chronic pelvic pain AUA 2025 male CPP statements; myofascial Abbott RCT and FitzGerald numbers; constipation trial response rates and OIC section; defecatory dysfunction 37% median finding corrected; fecal incontinence SNM durability re-sourced and CONFIDeNT/NOTABLE added.

---

## 2026-09-24 (later 20) — Audit v2 tier 2, batch 10: upper-tract conditions and the fistula section

- **Checked:** NLUTD: [Spinal Cord Injury](docs/03-clinical-conditions/03d-nlutd/nlutd-spinal-cord-injury.mdx), [Stroke & Dementia](docs/03-clinical-conditions/03d-nlutd/nlutd-stroke-dementia.mdx); upper tract: [Retroperitoneal Fibrosis](docs/03-clinical-conditions/03e-upper-tract/retroperitoneal-fibrosis.mdx), [UPJ Obstruction](docs/03-clinical-conditions/03e-upper-tract/upj-obstruction.mdx), [Ureteral Stricture](docs/03-clinical-conditions/03e-upper-tract/ureteral-stricture.mdx); fistulas: [index](docs/03-clinical-conditions/03f-fistulas/index.mdx), all-patients pages (colovesical/small bowel, nephropleural, post-kidney-transplant, pyeloenteric, ureterocolonic, vascular-urinary, vesicocutaneous), in-females pages (obstetric, rectovaginal, ureterovaginal, vesicouterine, vesicovaginal), in-males pages (rectourethral, rectovesical, urethrocutaneous, urethroperineal, urethropubic).
- **Main corrections:** SCI: spinal shock and surveillance statements, SNM AUA/EAU difference stated; RPF: mycophenolate figures re-cited, relapse vs remission predictors, imaging displacement scoped; ureteral stricture: metallic stent and endoureterotomy figures replaced, unsourced reimplant/psoas rates removed; UPJ: incidence, endopyelotomy HR comparator, redo pyeloplasty; fistulas: obstetric prevalence figures marked as widely quoted, catheter 7 vs 14 days RCT, VVF hysterectomy and radiation figures, radiation interposition EAU claim corrected on the hub, RVF ERAF scope and gracilis pooled outcomes, rectourethral risk figures re-attributed, colovesical Bourne/Crohn EVF figures scoped, vascular-urinary iliac table, ureterovaginal stent success and timing, vesicouterine age and citation fixes, urethroperineal AUA urethrostomy wording, urethropubic CT claims and outcomes with denominators.

---

## 2026-09-24 (later 19) — Audit v2 tier 2, batch 9: voiding outlet, pelvic support and NLUTD clinical conditions

- **Checked:** voiding/storage conditions: [Nocturia](docs/03-clinical-conditions/03a-storage-incontinence/nocturia.mdx), [Dysfunctional Voiding](docs/03-clinical-conditions/03b-voiding-outlet/dysfunctional-voiding.mdx), [Female Urethral Stricture](docs/03-clinical-conditions/03b-voiding-outlet/female-urethral-stricture.mdx), [Primary Bladder Neck Obstruction](docs/03-clinical-conditions/03b-voiding-outlet/primary-bladder-neck-obstruction.mdx), [Underactive Bladder](docs/03-clinical-conditions/03b-voiding-outlet/underactive-bladder.mdx), [Urethral Diverticula](docs/03-clinical-conditions/03b-voiding-outlet/urethral-diverticula.mdx), [Urethral Prolapse](docs/03-clinical-conditions/03b-voiding-outlet/urethral-prolapse.mdx), [Female Urethral Masses](docs/03-clinical-conditions/03b-voiding-outlet/female-urethral-masses.mdx) (`partial`: carcinoma/leiomyoma refs); pelvic support: [Anterior](docs/03-clinical-conditions/03c-pelvic-support/anterior-compartment.mdx), [Apical](docs/03-clinical-conditions/03c-pelvic-support/apical-prolapse.mdx), [Posterior](docs/03-clinical-conditions/03c-pelvic-support/posterior-compartment.mdx), [Obstetric Perineal Injury](docs/03-clinical-conditions/03c-pelvic-support/obstetric-perineal-injury.mdx); NLUTD: [index](docs/03-clinical-conditions/03d-nlutd/index.mdx), [Neurogenic Bladder](docs/03-clinical-conditions/03d-nlutd/neurogenic-bladder.mdx), [Autonomic Dysreflexia](docs/03-clinical-conditions/03d-nlutd/autonomic-dysreflexia.mdx), [Cauda Equina](docs/03-clinical-conditions/03d-nlutd/nlutd-cauda-equina.mdx) (restored from 454 to ~1,400 words), [Multiple Sclerosis](docs/03-clinical-conditions/03d-nlutd/nlutd-multiple-sclerosis.mdx), [Parkinson](docs/03-clinical-conditions/03d-nlutd/nlutd-parkinson.mdx), [Spina Bifida](docs/03-clinical-conditions/03d-nlutd/nlutd-spina-bifida.mdx) (`partial`: later sections not reached).
- **Main corrections:** autonomic dysreflexia acute drug table per Consortium guideline (nifedipine swallowed not sublingual, hydralazine oral, nitrate/PDE5 intervals, tachycardia not bradycardia); anterior compartment: OR 3.99 scoped to primary prolapse, mis-cited refs replaced, Cochrane 2026 numbers; obstetric injury: fabricated-looking meta-analysis ORs replaced with verified RRs; posterior: Ferrari 2022 mischaracterization, Cochrane transanal vs transvaginal; posterior/apical additions (Paraiso, FDA 2019, SUPeR); urethral diverticula/masses: MRI accuracy figures, denominators for recurrence and de novo SUI, Leach classification; bladder neck obstruction: BNI misattributed ejaculation rates and citation fixed; female stricture: incidence mix-up corrected; MS: EAU prevalence and urodynamic patterns, beta-3 recommendation; PD: PVR >150 mL MSA discriminator; spina bifida: Botox pediatric dose source, cancer-risk claim removed, mirabegron pediatric approval.

---

## 2026-09-24 (later 18) — Audit v2 tier 2, batch 8: sexual-medicine completion, storage/diversion/voiding-outlet pharmacology

- **Checked:** [Priapism Management](docs/01-foundations/pharmacology/sexual-medicine-andrology/priapism-management.mdx) (with restoration), [Testosterone Replacement](docs/01-foundations/pharmacology/sexual-medicine-andrology/testosterone-replacement.mdx), [Sexual Medicine index](docs/01-foundations/pharmacology/sexual-medicine-andrology/index.mdx), [Peyronie's Disease Agents](docs/01-foundations/pharmacology/sexual-medicine-andrology/peyronies-disease-agents.mdx) (restored ~6,000 words from baseline, 46 new refs); [Desmopressin](docs/01-foundations/pharmacology/storage-oab/desmopressin.mdx), [Storage-OAB index](docs/01-foundations/pharmacology/storage-oab/index.mdx); urinary diversion: [index](docs/01-foundations/pharmacology/urinary-diversion-specific/index.mdx), [Mucus Management](docs/01-foundations/pharmacology/urinary-diversion-specific/mucus-management.mdx), [Urinary Acidifiers](docs/01-foundations/pharmacology/urinary-diversion-specific/urinary-acidifiers.mdx), [Vitamin B12](docs/01-foundations/pharmacology/urinary-diversion-specific/vitamin-b12-supplementation.mdx); voiding outlet: [5-ARIs](docs/01-foundations/pharmacology/voiding-outlet/5-alpha-reductase-inhibitors.mdx), [Alpha-Agonists](docs/01-foundations/pharmacology/voiding-outlet/alpha-agonists.mdx) (placeholder built out), [Alpha-Blockers](docs/01-foundations/pharmacology/voiding-outlet/alpha-blockers.mdx), [Cholinergic Agonists](docs/01-foundations/pharmacology/voiding-outlet/cholinergic-agonists.mdx), [Skeletal Muscle Relaxants](docs/01-foundations/pharmacology/voiding-outlet/skeletal-muscle-relaxants.mdx), [Voiding Outlet index](docs/01-foundations/pharmacology/voiding-outlet/index.mdx) (`partial`). Dantrolene recorded `partial` (unbuilt placeholder).
- **Main corrections:** testosterone: 2025 removal of boxed BP warnings (Xyosted, oral undecanoates), Xyosted/Tlando doses, gel secondary-exposure wording, TRAVERSE formulation scope; desmopressin: Nocdurna/Noctiva US availability, Cochrane figures, label contraindications; storage-OAB hub: wrong AUA guideline PMID link fixed; acidifiers: incidence, methenamine, chlorpromazine scoping; B12: Indiana pouch source (Terai), unsupported 15-25% range removed, replacement regimens; mucus: octreotide study design, ranitidine withdrawal; alpha-blockers: IFIS, TWOC statement 57, ejaculation and dementia; Peyronie's: label AE rates, Cocci/Lipshultz/Sadagopan/Wegner/Cilio mischaracterizations, wrong Anelli PMID; priapism: technique and evidence restored with baseline errors corrected.

---

## 2026-09-24 (later 17) — Audit v2 tier 2, batch 7: perioperative ERAS pharmacology and sexual-medicine agents

- **Checked:** perioperative ERAS: [index](docs/01-foundations/pharmacology/perioperative-eras/index.mdx), [Anticoagulation Reversal](docs/01-foundations/pharmacology/perioperative-eras/anticoagulation-reversal.mdx), [Bowel Preparation](docs/01-foundations/pharmacology/perioperative-eras/bowel-preparation.mdx), [Corticosteroids](docs/01-foundations/pharmacology/perioperative-eras/corticosteroids.mdx), [Postoperative Ileus](docs/01-foundations/pharmacology/perioperative-eras/postop-bowel-ileus-management.mdx), [VTE Prophylaxis](docs/01-foundations/pharmacology/perioperative-eras/vte-prophylaxis.mdx); sexual medicine: [Androgen Adjuncts](docs/01-foundations/pharmacology/sexual-medicine-andrology/androgen-adjuncts.mdx), [Bremelanotide](docs/01-foundations/pharmacology/sexual-medicine-andrology/bremelanotide.mdx), [Female Testosterone (HSDD)](docs/01-foundations/pharmacology/sexual-medicine-andrology/female-testosterone-hsdd.mdx), [Flibanserin](docs/01-foundations/pharmacology/sexual-medicine-andrology/flibanserin.mdx), [Intracavernosal Injection Agents](docs/01-foundations/pharmacology/sexual-medicine-andrology/intracavernosal-injection-agents.mdx), [Intraurethral Alprostadil](docs/01-foundations/pharmacology/sexual-medicine-andrology/intraurethral-alprostadil.mdx), [PDE5 Inhibitors](docs/01-foundations/pharmacology/sexual-medicine-andrology/pde5-inhibitors.mdx), [Peyronie's Disease Agents](docs/01-foundations/pharmacology/sexual-medicine-andrology/peyronies-disease-agents.mdx). Restoration reviews against baseline `e81c5b0d` restored verified content on androgen adjuncts, intracavernosal agents, intraurethral alprostadil and PDE5 inhibitors.
- **Main corrections:** anticoagulation: INCH trial figures, idarucizumab and andexanet claims, vitamin K dose, bridging/interruption added; bowel prep: Shafii/Aslan/Hashad descriptions, sodium phosphate contraindications, MOBILE/ORALEV added; corticosteroids: PADDI PONV secondary outcomes, stress-dose row, diabetes and label cautions; ileus: metoclopramide duration limit scoped, methylnaltrexone/erythromycin/lidocaine/NG evidence; VTE: enoxaparin regimen note and urogynecologic section; index hub statements aligned; androgen adjuncts: AUA Table 6 doses, clomiphene VTE wording, Habous; bremelanotide: RECONNECT SSE result; female testosterone: Global Consensus endorsers, dosing, DHEA and Gompel claims; flibanserin: label adverse-event table, REMS date; intracavernosal: Caverject vs Edex combination wording; MUSE and PDE5 label detail (avanafil food effect, AUA statements 8-12); Peyronie's: Cochrane CI, IMPRESS in degrees, ESWT/traction section. Also `storage-oab/botulinum-toxin` 300 U attribution (batch 6).

---

## 2026-09-24 (later 16) — Audit v2 tier 2, batch 6: legacy agents, neuromodulation adjuncts, neuropathic pelvic pain pharmacology

- **Checked:** [Sodium Fluorescein](docs/01-foundations/pharmacology/intraoperative-adjuncts/visualization-agents/sodium-fluorescein.mdx); legacy: [index](docs/01-foundations/pharmacology/legacy-low-evidence/index.mdx), [Imipramine](docs/01-foundations/pharmacology/legacy-low-evidence/imipramine.mdx), [Parasympathomimetics](docs/01-foundations/pharmacology/legacy-low-evidence/parasympathomimetics.mdx), [Phenazopyridine](docs/01-foundations/pharmacology/legacy-low-evidence/phenazopyridine.mdx), [Phytotherapy](docs/01-foundations/pharmacology/legacy-low-evidence/phytotherapy.mdx); [Neuromodulation Adjuncts index](docs/01-foundations/pharmacology/neuromodulation-adjuncts/index.mdx) and [Botulinum Toxin](docs/01-foundations/pharmacology/neuromodulation-adjuncts/botulinum-toxin.mdx); neuropathic pelvic pain: [Gabapentinoids](docs/01-foundations/pharmacology/neuropathic-pelvic-pain/gabapentinoids.mdx), [Local Anesthetics](docs/01-foundations/pharmacology/neuropathic-pelvic-pain/local-anesthetics.mdx), [NSAIDs & Analgesics](docs/01-foundations/pharmacology/neuropathic-pelvic-pain/nsaids-analgesics.mdx), [SNRIs](docs/01-foundations/pharmacology/neuropathic-pelvic-pain/snris.mdx), [Topical Compounded Agents](docs/01-foundations/pharmacology/neuropathic-pelvic-pain/topical-compounded-agents.mdx), [Tricyclic Antidepressants](docs/01-foundations/pharmacology/neuropathic-pelvic-pain/tricyclic-antidepressants.mdx). Also corrected the 300 U vs 200 U attribution on [storage-oab Botulinum Toxin](docs/01-foundations/pharmacology/storage-oab/botulinum-toxin.mdx). The neuropathic pain index and Antispasmodics (unbuilt placeholder) are recorded `partial`.
- **Main corrections:** fluorescein dose/efflux/discoloration from label and trials; imipramine MAOI reaction, NICE NG123 position; bethanechol label table and EAU 2026 do-not-prescribe; phenazopyridine label limits and efflux evidence; phytotherapy pumpkin-seed regimen and guideline wording; botulinum: AUA Statement 25 wording, label AE rates replacing unsupported ranges; gabapentinoids: Cochrane CPPS and AUA IC/BPS wording, label comparison; local anesthetics: epinephrine dose limits, ASRA lipid rescue, Parsons 2012 corrected; NSAIDs: Lv 2014, Cochrane n, unverifiable network figures replaced, ketorolac bleeding contraindications, opioid/tramadol label safety; SNRIs: Cochrane SUI result, Ghoniem/Steers/Sanchez-Salas details; TCAs: Di 2021 scope, vulvodynia negative trials; topical agents: Goetsch RCT attribution, Poterucha denominator, capsaicin label scope.

---

## 2026-09-24 (later 15) — Audit v2 tier 2, batch 5: sealants, tranexamic acid, visualization agents

- **Checked:** [PEG Sealants](docs/01-foundations/pharmacology/intraoperative-adjuncts/tissue-sealants/peg-sealants.mdx), [TachoSil](docs/01-foundations/pharmacology/intraoperative-adjuncts/tissue-sealants/tachosil.mdx), [Tranexamic Acid](docs/01-foundations/pharmacology/intraoperative-adjuncts/tranexamic-acid.mdx), [ICG](docs/01-foundations/pharmacology/intraoperative-adjuncts/visualization-agents/icg.mdx), [Visualization Agents index](docs/01-foundations/pharmacology/intraoperative-adjuncts/visualization-agents/index.mdx), [Indigo Carmine](docs/01-foundations/pharmacology/intraoperative-adjuncts/visualization-agents/indigo-carmine.mdx), [Methylene Blue](docs/01-foundations/pharmacology/intraoperative-adjuncts/visualization-agents/methylene-blue.mdx), [Pudexacianinium](docs/01-foundations/pharmacology/intraoperative-adjuncts/visualization-agents/pudexacianinium.mdx).
- **Main corrections:** PEG: wrong Baxter reference replaced, CoSeal cure/resorption/swelling from IFU and FDA SSED, unverified comparison rows removed; TachoSil: label indication and overlap, Peyronie's collagen-fleece wording, EU indication, efficacy trials; TXA: label renal table, infusion rate, TURP data re-sourced, upper-tract clot case reports; ICG: label fluorescence timing, off-label lymphatic scope, 2026 ureteral and anastomotic-leak meta-analyses; indigo carmine: label version pointer, efflux 4-9 min, pregnancy wording, cystoscopy detection evidence; methylene blue: current label date, WSES ureter data by fluorescence, false MAO-inhibition claim not restored, low-dose serotonin case reports; pudexacianinium: phase III trials listed, renal PK, scale validation; hub: WSES methylene blue contraindications and fluorescein dose.

---

## 2026-09-24 (later 14) — New sources applied: EAU Urological Infections 2026, AUA Table V, ASHP 2013, hemostat IFUs

- **Infection pages:** [Antifungals](docs/01-foundations/pharmacology/infection-prophylaxis/antifungals.mdx), [Non-Antibiotic UTI Prevention](docs/01-foundations/pharmacology/infection-prophylaxis/non-antibiotic-uti-prevention.mdx), [UTI Suppressive Prophylaxis](docs/01-foundations/pharmacology/infection-prophylaxis/uti-suppressive-prophylactic.mdx), [UTI Treatment Antibiotics](docs/01-foundations/pharmacology/infection-prophylaxis/uti-treatment-antibiotics.mdx), [Perioperative Antibiotic Prophylaxis](docs/01-foundations/pharmacology/infection-prophylaxis/perioperative-antibiotic-prophylaxis.mdx), [Prosthetic Infection & Biofilm](docs/01-foundations/pharmacology/infection-prophylaxis/prosthetic-infection-biofilm.mdx). EAU statements now read from the guideline text; fosfomycin prophylaxis interval and a misattributed prostatitis citation corrected; Table V agents added per procedure; Barham vancomycin-gentamicin regimen scoped as one Table V option; gentamicin dosing weight per ASHP.
- **Hemostats:** [FloSeal](docs/01-foundations/pharmacology/intraoperative-adjuncts/hemostatic-agents/floseal.mdx) now compares the human-thrombin and Recothrom kits from their IFUs (mixing, precautions, swelling, application); [Gelfoam](docs/01-foundations/pharmacology/intraoperative-adjuncts/hemostatic-agents/gelfoam.mdx), [Arista](docs/01-foundations/pharmacology/intraoperative-adjuncts/hemostatic-agents/arista.mdx), [Surgicel](docs/01-foundations/pharmacology/intraoperative-adjuncts/hemostatic-agents/surgicel.mdx) and the [hub](docs/01-foundations/pharmacology/intraoperative-adjuncts/hemostatic-agents/index.mdx) aligned to labels (Surgiflo collecting-system warning, Arista swelling/applicators, Surgicel absorption wording and SNoW description; unsourced cost figures removed).

---

## 2026-09-24 (later 13) — Audit v2 tier 2, batch 4: intraoperative adjuncts

- **Checked:** [Surgicel](docs/01-foundations/pharmacology/intraoperative-adjuncts/hemostatic-agents/surgicel.mdx), [Topical Thrombin](docs/01-foundations/pharmacology/intraoperative-adjuncts/hemostatic-agents/topical-thrombin.mdx), [Hydrodissection](docs/01-foundations/pharmacology/intraoperative-adjuncts/hydrodissection-agents.mdx), [Intraoperative Adjuncts index](docs/01-foundations/pharmacology/intraoperative-adjuncts/index.mdx), [Liposomal Bupivacaine](docs/01-foundations/pharmacology/intraoperative-adjuncts/liposomal-bupivacaine.mdx), [Cyanoacrylates](docs/01-foundations/pharmacology/intraoperative-adjuncts/tissue-sealants/cyanoacrylates.mdx), [Fibrin Sealants](docs/01-foundations/pharmacology/intraoperative-adjuncts/tissue-sealants/fibrin-sealants.mdx), [Tissue Sealants index](docs/01-foundations/pharmacology/intraoperative-adjuncts/tissue-sealants/index.mdx). FloSeal and the hemostatic-agents index edited to describe both human-thrombin and Recothrom kits.
- **Main corrections:** Surgicel: mechanism, dry application, thrombin incompatibility from the IFU; thrombin: bovine boxed warning, intravascular wording, per-product doses, trial hemostasis rates replacing an unsourced onset; hydrodissection: vasopressin off-label dosing, epinephrine end-artery label wording, Campagna citation removed from unrelated claims; liposomal bupivacaine: Sandhu, Schmidt and Zhang trial descriptions, Zeng 2026, Baby ORIOLES and Pence added; cyanoacrylates: LiquiBand/Indermil composition, contraindications, off-label use; fibrin sealants: label composition (factor XIII removed), spray parameters, thawing, Evicel/Vistaseal specifics.

---

## 2026-09-24 (later 12) — Audit v2 tier 2, batch 3: infection prophylaxis, hemostatic agents; new GSM and infection sources applied

- **New local sources (user-supplied):** AUA/SUFU/AUGS GSM 2025, NAMS 2020 GSM, ACOG Clinical Consensus 2 (2021), IDSA candidiasis 2016, AUA BPS antimicrobial prophylaxis 2019, WikiGuidelines UTI 2024, Cochrane cranberries. Used to close open items on [Vaginal Moisturizers](docs/01-foundations/pharmacology/hormonal-therapies/vaginal-moisturizers.mdx), [Vaginal Topical Estrogen](docs/01-foundations/pharmacology/hormonal-therapies/vaginal-topical-estrogen.mdx), [Vaginal DHEA](docs/01-foundations/pharmacology/hormonal-therapies/vaginal-dhea.mdx), [Ospemifene](docs/01-foundations/pharmacology/hormonal-therapies/ospemifene.mdx), the hormonal-therapies index, [Antifungals](docs/01-foundations/pharmacology/infection-prophylaxis/antifungals.mdx), the infection-prophylaxis index and [Non-Antibiotic UTI Prevention](docs/01-foundations/pharmacology/infection-prophylaxis/non-antibiotic-uti-prevention.mdx).
- **Checked (new):** [Perioperative Antibiotic Prophylaxis](docs/01-foundations/pharmacology/infection-prophylaxis/perioperative-antibiotic-prophylaxis.mdx), [Prosthetic Infection & Biofilm](docs/01-foundations/pharmacology/infection-prophylaxis/prosthetic-infection-biofilm.mdx), [UTI Suppressive Prophylaxis](docs/01-foundations/pharmacology/infection-prophylaxis/uti-suppressive-prophylactic.mdx), [UTI Treatment Antibiotics](docs/01-foundations/pharmacology/infection-prophylaxis/uti-treatment-antibiotics.mdx), [Arista](docs/01-foundations/pharmacology/intraoperative-adjuncts/hemostatic-agents/arista.mdx), [FloSeal](docs/01-foundations/pharmacology/intraoperative-adjuncts/hemostatic-agents/floseal.mdx), [Gelfoam](docs/01-foundations/pharmacology/intraoperative-adjuncts/hemostatic-agents/gelfoam.mdx), [Hemostatic Agents index](docs/01-foundations/pharmacology/intraoperative-adjuncts/hemostatic-agents/index.mdx).
- **Main corrections:** GSM: moisturizer schedule (NAMS), ACOG "should be considered first-line", serum estradiol ranges, NAMS rUTI wording; antifungals: funguria drug/duration from IDSA not AUA, neutropenia wording; non-antibiotic: Cochrane cranberry PAC has no dose threshold, WikiGuidelines hydration softened; periop prophylaxis: fluoroquinolone cautions, urodynamics exception, BPS wording; prosthetic infection: microbiology from Gross 2017, InhibiZone handling, salvage outcomes; suppressive prophylaxis: fluoroquinolone re-sourced, ImpresU figures, nitrofurantoin risk; UTI treatment: Kim 2020 network meta-analysis restated, renal dosing table, ASB attribution fixed; hemostats: wrong Arista and FloSeal references replaced, label warnings and 2026 partial-nephrectomy evidence added.

---

## 2026-09-24 (later 11) — Audit v2 tier 2, batch 2: vaginal hormones, pharmacology index, infection prophylaxis

- **Checked:** [Vaginal Moisturizers](docs/01-foundations/pharmacology/hormonal-therapies/vaginal-moisturizers.mdx), [Vaginal Topical Estrogen](docs/01-foundations/pharmacology/hormonal-therapies/vaginal-topical-estrogen.mdx), [Pharmacology index](docs/01-foundations/pharmacology/index.mdx) (no edits), [Antifungals](docs/01-foundations/pharmacology/infection-prophylaxis/antifungals.mdx), [Infection Prophylaxis index](docs/01-foundations/pharmacology/infection-prophylaxis/index.mdx), [Non-Antibiotic UTI Prevention](docs/01-foundations/pharmacology/infection-prophylaxis/non-antibiotic-uti-prevention.mdx).
- **Main corrections:** moisturizers: Hickey 2016 (randomized crossover, primary endpoint not significant), Loprinzi 1997 and Goetsch 2015 figures, Dezzutti/Palacios scoping, AUA GSM 2025 grades; estrogen: AUA rUTI pooled RR 0.58 is 2025 (not 2022), Grade B wording, Estrace/Imvexxy/Vagifem/Estring label doses and contraindications, Srinivasan and Moore/Shen scoping, EAU 2026 and NICE positions; antifungals: neutropenia-with-obstruction moved out of single-dose prophylaxis, fluconazole preferred agent, renal dosing and interactions from label, AmB irrigation dose; infection index: TOCUS OR scoped, Tan-Kim and Jent labeled; non-antibiotic: AUA statement numbers, EAU 2026 strong/weak positions, cranberry PAC scoping, several trial p-values and comparators corrected.

---

## 2026-09-25 — Audit v2 tier 2, batch 1: dermatologic/urethral and hormonal pharmacology

- **Checked:** [Dermatologic & Topical index](docs/01-foundations/pharmacology/dermatologic-topical-urethral/index.mdx), [Drug-Coated Balloon Therapy](docs/01-foundations/pharmacology/dermatologic-topical-urethral/drug-coated-balloon-therapy.mdx), [High-Potency Topical Corticosteroids](docs/01-foundations/pharmacology/dermatologic-topical-urethral/high-potency-topical-corticosteroids.mdx), [HPV Topical Agents](docs/01-foundations/pharmacology/dermatologic-topical-urethral/hpv-topical-agents.mdx), [Intralesional Corticosteroids](docs/01-foundations/pharmacology/dermatologic-topical-urethral/intralesional-corticosteroids.mdx), [Platelet-Rich Plasma](docs/01-foundations/pharmacology/dermatologic-topical-urethral/platelet-rich-plasma.mdx), [Topical Calcineurin Inhibitors](docs/01-foundations/pharmacology/dermatologic-topical-urethral/topical-calcineurin-inhibitors.mdx), [Hormonal Therapies index](docs/01-foundations/pharmacology/hormonal-therapies/index.mdx), [Gender-Affirming Hormone Therapy](docs/01-foundations/pharmacology/hormonal-therapies/gender-affirming-hormone-therapy.mdx), [Ospemifene](docs/01-foundations/pharmacology/hormonal-therapies/ospemifene.mdx), [Preoperative Hormonal Priming](docs/01-foundations/pharmacology/hormonal-therapies/preoperative-hormonal-priming.mdx), [Vaginal DHEA](docs/01-foundations/pharmacology/hormonal-therapies/vaginal-dhea.mdx).
- **Main corrections:** DCB: preclinical and Jelisejevas figures, EAU strong recommendation and label sizing added; steroids: Myers 2006, fingertip-unit guidance and Lee 2015 adherence data; HPV: sinecatechins and imiquimod efficacy figures, podofilox pregnancy wording; intralesional: Funaro and Mateu corrections, Soliman meta-analysis; PRP: platelet concentration, off-label 510(k) status, Jiang and Eryilmaz study designs; calcineurin: label strengths, boxed-warning source, malignancy evidence (Adams 2026, JOELLE); GAHT: ACOG citation, perioperative hormone-hold evidence, spironolactone monitoring; ospemifene: label-verified with meta-analysis figures fixed and AUA/SUFU/AUGS 2025 statements added; vaginal DHEA: trial table attribution, rUTI RR re-sourced to Chen 2021, breast-cancer label wording; priming: IMPROVE and Rahn tissue-change scoping.
...
Complete traditional-sling/open/laparoscopic Cochrane mains and all 342 actual graphics are read, with additional original mains and separately tracked inaccessible figures/videos/supplements. All local checks and 20 visual checks pass; output 131.46 MB. Ledger: 670 full current MDX reads, 640 scoped updates, 14 unresolved, 511 unreviewed. Content `dd4eafee` passed exact-commit CI/Vercel and all twelve live checks; see the [release record](reports/2026-09-12/twenty-third-continuation-release.md). Continue the whole-site task with the active sacrocolpopexy/ASPIRe source review.
...
Six new complete source mains are read, with figures and supplements accounted for separately. All 244 inherited references are preserved in the [source/access review](reports/2026-09-12/clamps-protocol-source-review.md). Ledger: 650 current full MDX reads, 626 scoped updates; three flap pages remain only partially reviewed. See the [seventeenth release record](reports/2026-09-12/seventeenth-continuation-release.md) for validation and exact publication status. Continue the whole-site task after publication.
...
Corrected ten bulking-product pages, the operative companion, nine genital-reconstruction pages and eleven fistula techniques plus two clinical companions. Changes separate current versus historical device formulations, actual indications and endpoint denominators; remove unsupported graft/substitute advantages and unsafe pannus-suspension advice; and align urethrocutaneous/urosymphyseal anatomy, evidence and selection.

Lint, typecheck, 32 component tests, 50 maintenance tests and the production build pass. All 99,044 compiled links/assets and 730 data links pass. Output **134.39 MB**. All 37 changed routes plus four interaction checks pass; five visual samples were checked. [Sixth release record](reports/2026-09-12/sixth-continuation-release.md) tracks publication. Continue the entire-site review after this release; evidence stays quarterly and Epic stays paused.
...
Source lint, typecheck, 32 component tests, 50 maintenance tests and production build pass; output is **136.30 MB**. A stapler heading-format build error was corrected while preserving its older anchor. [Fourth release record](reports/2026-09-12/fourth-continuation-release.md) tracks browser checks and publication. Review continues after this intermediate release.
...
[Release record](reports/2026-09-12/second-continuation-release.md) tracks validation and publication. Remaining device, fistula, other surgical/foundation, assessment, History and resource reviews continue after this publication. Evidence remains quarterly; Epic stays paused.
...
**Video-library refresh.** Ran `npm run videos:sync` against the WARWIKI YouTube channel. [videos.ts](src/data/videos.ts) now reflects **140 playlists / 1,542 unique videos**. The Resources copy was updated from "90+ custom playlists" to **"140+ custom playlists"** in [Resources](docs/08-resources/index.mdx) and [Videos & Surgical Atlases](docs/08-resources/surgical-video-atlases.mdx). `src/data/stats.json` was regenerated by `npm run build`.

**VideoCards generalized.** [VideoCards.tsx](src/components/VideoCards.tsx) now supports direct non-YouTube video sources with optional `sourceUrl` / `sourceType`, while preserving the existing lazy YouTube thumbnail/embed path. [custom.css](src/css/custom.css) adds direct-video placeholder styling and an "Open source video" link style.
...
## 2026-06-16 — Social: patient-handouts language announcement assets

Created/finalized an off-repo social package at `~/Desktop/WARWIKI-social/handouts-languages/` for announcing that WARWIKI has **80 patient guides in 10 languages**. A local `HANDOFF.md` now indexes the final files.

**Instagram single-post asset.** Final image: `single-better.png` (`2160x2700`), generated from `single-better.js` with editable `single-better.svg` and browser source `single-better.html`. The final design is a concise portrait announcement: "80 patient guides in 10 languages," a short "Free, printable, plain-language PDFs." support line, large language/flag tiles, and a single CTA pill ("Read & print free -> warwiki.org"). Removed redundant copy, the bottom footer line, and the "10 READY" badge; enlarged the flags; switched the WARWIKI wordmark toward the website/social-card sans branding. Caption saved in `caption.txt`; opening line: "Take patient education to the next level with brand-new official WARWIKI patient handouts, now available in 10 languages."

**Twitter / X adaptation.** Final image: `twitter.png` (`3200x1800`), generated from `twitter.js` with editable `twitter.svg` and browser source `twitter.html`. Landscape 16:9 layout: headline + CTA on the left, language panel on the right. Fixed Arabic RTL label placement in the SVG/Chrome render path. Tweet copy and alt text saved in `twitter-caption.txt`; the tweet draft is under the X character limit.

**Durable workflow notes.** Social assets remain outside the repo under `~/Desktop/WARWIKI-social/`, matching the prior Instagram-carousel convention. Generator pattern is Node -> SVG-in-HTML -> headless Chrome screenshot at 2x scale for upload-safe PNGs. Language set shown: Spanish, Mandarin, Vietnamese, French, Korean, Tagalog, Arabic, Russian, Cantonese, Hindi.
...
- **Peyronie's curvature correction** (`peyronie-correction.js`, #42) — convex-side plication/Nesbit vs concave-side incision+graft, with the curve/length/rigidity decision. → [Peyronie's Disease](docs/04-surgical-techniques/04j-sexual-dysfunction/peyronies-disease/index.mdx).
- **Overlapping anal sphincteroplasty** (`sphincteroplasty.js`, #43) — EAS defect → overlapped (vest-over-pants) repair with mattress sutures + overlap detail inset. → [Anal Sphincteroplasty](docs/04-surgical-techniques/04h-fistula-repair/female/anal-sphincteroplasty.mdx).
- **Indiana pouch** (`indiana-pouch.js`, #44) — detubularized right-colon reservoir + tapered ileal limb + ileocecal-valve continence to a catheterizable stoma (continent cutaneous diversion, vs the orthotopic Studer neobladder). → [Indiana Pouch](docs/04-surgical-techniques/04c-urinary-diversion/indiana-pouch.mdx).
...
**Also this day — new page: [Heineke-Mikulicz Technique](docs/01-foundations/surgical-principles/flaps/heineke-mikulicz.mdx)** (1184 → 1185 pages). User asked for a dedicated section; the two source dumps were ~half GI surgery (pyloroplasty, Crohn's strictureplasty, Finney/Michelassi, dumping) — **out of scope as primary topics**, so the page was **reframed for reconstructive urology/urogyn**: origin kept to two sentences, GU applications carry it. Placed in `surgical-principles/flaps/` beside its geometric siblings Z-plasty / Y-V plasty (hidden category → reached by inbound links), reusing the existing HM schematic. Covers: male stricture (Lumen 2010 non-transecting "stricturoplasty" + Kanematsu 2025 double-sided), female meatoplasty, ureter/UPJ (Fenger), genital skin (preputioplasty, penoscrotal-web scrotoplasty), urethral diverticulum, and gender-affirming. **Every dump paper verified by lit search first** — caught the dump's truncated Lumen criterion (actually **&lt;1 cm**, not "&lt;2 cm"), confirmed Kanematsu 2025 (`iju.70108`) is legitimate and distinct from the invagination Kanematsu paper dropped on 2026-06-04, and added two extra verified applications the dump missed (HM preputioplasty — Xu/Mishra/Zhao 2022; HM scrotoplasty — Elrouby 2024). 11 refs with stable parallel anchors. Inbound links wired from non-transecting-bulbar (the schematic caption), pyeloplasty (Fenger), ureteroureterostomy, female-urethral-stricture, and Y-V plasty (orphan lint clean at 371 linked hidden pages). Lesson reinforced: **verify dump-named papers even when a DOI is supplied** — a same-author same-year paper can be a different study.

**Also this day — figure-caption rendering fix + permanent guard.** User reported the Martius caption "cut off" and the transureteroureterostomy image/caption "bleeding into one another." Root cause: the house embed put the `![]()` image and its `*caption*` on adjacent lines with **no blank line between them**, so CommonMark kept them in **one `<p>`**; an inline `<img>` lets the caption flow *beside* the image on wide content columns (it only shows when the column is wider than the figure, which is why it was missed). Confirmed with a side-by-side render (same paragraph vs blank-line-separated), then **swept all 67 `/img/` embeds across 53 pages** (this session's + the prior 16 + the 6 PD plates + a stray `wound-healing` figure), inserting a blank line so each image is its own paragraph. **Verified live** (served the build, loaded both flagged pages): the image now closes its `</p>` before the caption `<p>`, and the caption renders below. **Guard added so it can't recur:** `scripts/check-figure-captions.js` (`npm run lint:figures`, wired into `npm run lint`) fails if any `/img/` embed isn't followed by a blank line (negative-tested). Corrected the CLAUDE.md embed-pattern convention from "immediately followed by" → "blank line then" the caption (durable + live entries); saved [[feedback_figure_caption_blank_line]]. **Follow-up (same day) — the blank line wasn't enough:** the user reported TUU still "bleeding" / "cuts off." Second root cause: every this-session figure also carried a **dense 1–3-line prose legend inside the SVG**, which — when the 820-px figure scales into the ~600-px doc column — becomes tiny/unreadable and visually merges with the caption right below it. That legend only duplicated the markdown caption + page body, so **removed it from all 29 affected generators and cropped each figure's height to the legend divider** (`H = dividerY + 6`); re-rendered and QA'd (legends gone, no diagram/callout-box content clipped; live TUU now 605×294 with the readable caption below). The 15 prior-session figures use short labels / side boxes and were unaffected. **Durable lesson: keep explanatory prose in the markdown caption, NOT as small print inside a figure that will be downscaled** — a scaled-down SVG legend is unreadable. aria-labels still carry the full description.
