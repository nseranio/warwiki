...
**14 original SVG schematics (13 commits)** — all copyright-free, authored as re-runnable generator scripts in `scripts/diagrams/`, embedded with the house caption pattern ending **(Original WARWIKI schematic)**, build-validated:
...
**Still on the bench (remaining high-value candidates, each reuses an engine):** Blandy female vaginal-flap (Heineke-Mikulicz) geometry · RUG/VCUG positioning + stricture silhouette · perineum layered sagittal anatomy · pudendal-nerve course/Alcock's canal. Also pending: embedding the 14 already-vetted public-domain anatomy plates onto their matching anatomy pages (zero-risk win).
