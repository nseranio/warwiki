...
**16 original SVG schematics (15 commits)** — all copyright-free, authored as re-runnable generator scripts in `scripts/diagrams/`, embedded with the house caption pattern ending **(Original WARWIKI schematic)**, build-validated:
...
15. **RUG urethral silhouette** (`b57a904`) — the male anterior-urethra contrast column in the RPO view: segments (fossa navicularis, penile, bulbar, membranous = normal narrowing, prostatic) with a focal bulbar stricture and the spongiofibrosis that is not opacified, so RUG underestimates true length by ~1 cm at each shoulder. → RUG/VCUG.
16. **Pudendal-nerve course** (`fe75a0e`) — posterolateral course (S2–S4 roots → greater sciatic foramen below piriformis → hook around the ischial spine → lesser sciatic foramen → Alcock's canal on obturator internus → inferior rectal / perineal / dorsal branches), flagging the ischial-spine (SSLF suture zone + pudendal-block landmark) and Alcock's-canal (entrapment) risk zones. → Pelvic Neuroanatomy.
...
**Still on the bench (remaining high-value candidates, each reuses an engine):** Blandy female vaginal-flap (Heineke-Mikulicz) geometry · perineum layered sagittal anatomy · VCUG female "wine-glass" silhouette · sonourethrogram / CT-urogram phases. The remaining unused `static/img/anatomy/` plates are mostly mislabeled or redundant with existing figures — verify each before reuse.
